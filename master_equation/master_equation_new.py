...
def calc_reduced_T(T,reaction,T_P_min_max_dict):
...
def calc_reduced_P(P,reaction,T_P_min_max_dict):
...
def multiply_by_sensitivites(T_P_min_max_dict,array1,array_of_sensitivities,pressure_and_temp_array,reaction):
    ...
    sensitivity_multiplied_array = np.zeros((array_of_sensitivities.shape[0],array_of_sensitivities.shape[1],len(pressure_and_temp_array['pressure'])))
    tuple_list = []
    ...
    for sensitivity in np.nditer(array_of_sensitivities,order='C'):
        ...
        for p,value in enumerate(pressure_and_temp_array['temperature']):
            ...
            t_coef = chebyshev_specific_poly(i,calc_reduced_T(value,reaction,T_P_min_max_dict))
            temp_coef.append(t_coef)
            p_coef = chebyshev_specific_poly(j,calc_reduced_P(pressure_and_temp_array['pressure'][p],reaction,T_P_min_max_dict))
...
def slicing_out_reactions(exp,reaction_string,array):
    reactions_in_cti_file = exp['simulation']['reaction_equations']
    index_of_reaction_in_cti = reactions_in_cti_file.index(reaction_string)
    column_of_array = array[:,index_of_reaction_in_cti]
    column_of_array = column_of_array.reshape((column_of_array.shape[0],1))  
    return column_of_array
        
def map_to_alpha_parallel_function(arg):
            
    exp = arg[0]
    parsed_yaml_file = arg[1]
    master_equation_reactions = arg[2]
    sensitivty_dict = arg[3]
    T_P_min_max_dict = arg[4]
            
    i = exp['index']
    simulation = []
    single_experiment = []
    #print(parsed_yaml_file['moleFractionObservables'][0],parsed_yaml_file['concentrationObservables'][0],parsed_yaml_file['ignitionDelayObservables'])
    if parsed_yaml_file['moleFractionObservables'][0] != None or parsed_yaml_file['concentrationObservables'][0] != None or parsed_yaml_file['ignitionDelayObservables'][0] !=None:
        # print('hey')
        As = exp['ksens']['A']
        for xx,observable in enumerate(As):
            ...
            for reaction in master_equation_reactions:
                column = slicing_out_reactions(exp,reaction,observable)
                if re.match('[Ss]hock[- ][Tt]ube',exp['simulation_type']) and re.match('[Ss]pecies[- ][Pp]rofile',exp['experiment_type']):
                    single_reaction_array = array_reshape(multiply_by_sensitivites(T_P_min_max_dict,column,sensitivty_dict[reaction][0],exp['simulation']['pressureAndTemperatureToExperiment'][xx],reaction))
                elif re.match('[Ss]hock[- ][Tt]ube',exp['simulation_type']) and re.match('[Ii]gnition[- ][Dd]elay',exp['experiment_type']):
                    single_reaction_array = array_reshape(multiply_by_sensitivites(T_P_min_max_dict,column,sensitivty_dict[reaction][0],exp['simulation']['timeHistories'][0],reaction))
    ...
    if 'absorbance_observables' in list(exp.keys()):
        wavelengths = parsed_yaml_file['absorbanceCsvWavelengths']
        for k,wl in enumerate(wavelengths):
            temp = []
            observable_list = []
            for reaction in master_equation_reactions:
                column = slicing_out_reactions(reaction,exp['absorbance_ksens'][wl][0])
                single_reaction_array = array_reshape(multiply_by_sensitivites(T_P_min_max_dict,column,sensitivty_dict[reaction][0],exp['time_history_interpolated_against_abs'][wl],reaction))
                temp.append(single_reaction_array)
                observable_list.append(single_reaction_array)
            single_experiment.append(np.hstack((temp)))
            simulation.append(observable_list)

    return (i, np.vstack((single_experiment)))
...
class Master_Equation(object):
    ...
    def map_to_alpha(self,sensitivty_dict:dict,
        ...
        pickle_exp_dict_list = []
        for exp in exp_dict_list:
            pickle_exp_dict = {}
            pickle_exp_dict['index'] = exp['index']
            # if exp['ksens']:
            #     pickle_exp_dict['ksens'] = {}            
            #     if exp['ksens']['A']:
            #         pickle_exp_dict['ksens']['A'] = exp['ksens']['A']
            pickle_exp_dict['ksens'] = {}            
            pickle_exp_dict['ksens']['A'] = exp['ksens']['A']
            pickle_exp_dict['simulation_type'] = exp['simulation_type']
            pickle_exp_dict['experiment_type'] = exp['experiment_type']   
            pickle_exp_dict['simulation'] = {}
            if 'pressureAndTemperatureToExperiment' in list(exp['simulation'].keys()):
                if exp['simulation']['pressureAndTemperatureToExperiment']:
                    temp_list = []
                    for temp_df in exp['simulation']['pressureAndTemperatureToExperiment']:
                        temp_list.append({val:list(temp_df[val]) for val in list(temp_df.keys())})
                    pickle_exp_dict['simulation']['pressureAndTemperatureToExperiment'] = temp_list
            if 'timeHistories' in list(exp['simulation'].keys()):
                temp_list = []
                for temp_df in exp['simulation']['timeHistories']:
                    temp_list.append({val:list(temp_df[val]) for val in list(temp_df.keys())})
                pickle_exp_dict['simulation']['timeHistories'] = temp_list
            pickle_exp_dict['simulation']['reaction_equations'] = exp['simulation']['reaction_equations']
            pickle_exp_dict_list.append(pickle_exp_dict)
            
        # args = pickle_exp_dict_list
        args = [[pickle_exp_dict_list[i], parsed_yaml_file_list[i], master_equation_reactions, sensitivty_dict, self.T_P_min_max_dict] for i in range(len(exp_dict_list))]
        
        temp_list = []
        for i in tqdm(range(len(exp_dict_list)), desc='Building Master Equation s-Matrix',unit='experiments',colour='white'):
            arg = args[i]
            temp_list.append(map_to_alpha_parallel_function(arg))    
        
        # pool = multiprocessing.Pool(processes=multiprocessing.cpu_count())
        # temp_list = list(tqdm(pool.imap_unordered(map_to_alpha_parallel_function, args), 
        #                      total=len(exp_dict_list), file=sys.stdout, colour="white", desc="Building Master Equation s-Matrix", unit='experiments'))
        
        sorted_list = sorted(temp_list, key=lambda x: x[0])
        mapped_to_alpha_full_simulation = [item[1] for item in sorted_list]

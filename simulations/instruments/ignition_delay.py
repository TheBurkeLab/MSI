...
class ignition_delay(sim.Simulation):
    ...
    def run_single(self):
        ...
        #print(self.fullParsedYamlFile['simulationType'],'inside here 1')
        if re.match('[Ss]hock[ -][Tt]ube',self.fullParsedYamlFile['simulationType']):
            #print(self.fullParsedYamlFile['simulationType'],'inside here two')
        ...
        if self.kineticSens:   
                ...
                #sens=self.BFM_pool(delay,self.n_processors,simulation_type = '[Ss]hock[ -][Tt]ube' )
    ...
    def ig_Xmax(self,data,target):
        ...
        #XX = ((data['pressure'].values)/(8.31446261815324*data['temperature'].values))
        #print(XX)
    ...
    def ig_X_specific_value(self,data,target,value,unit=''):
        ...
        if unit =='':
            ...
            temperature_matrix = data['temperature'].values
            pressure_matrix = data['pressure'].values
            species_array = data[target].values
            
            concentration = np.true_divide(1,temperature_matrix.flatten())*pressure_matrix.flatten()
            concentration *= (1/(8.314e6))*species_array.flatten()     
            max_value_index_in_array = np.argmax(concentration) + 1
            slice_concentration = concentration[0:max_value_index_in_array]
            slice_time = tt[0:max_value_index_in_array]
            
            
        delay=[]

        delay = np.interp(value,slice_concentration,slice_time)
    ...
    def direct_ksens(self,nominal,dk=0.01,observable=['temperature']):
        ...
        #print(sens[:,0,0])
        ksens=np.zeros(temp_st.gas.n_reactions)
        for i in range(temp_st.gas.n_reactions):
            if i==0:
                self.gas.set_multiplier(1+0.001,0)
                temp_history=copy.deepcopy(self.run_shocktube().timeHistory)
                self.gas.set_multiplier(1.0,0)
            tempsen=sens[:,i,0]
            sens_add=0.0
    ...
    def BFM(self,nominal,simulation_type = ''):
         sens=np.zeros(self.gas.n_reactions)
         if re.match('^[Mm]ax [Dd]erivative$',self.target_type):
            if re.match('[Tt]emperature',self.target):
                ...
                for i in range(self.gas.n_reactions):
                    ...
                    self.gas.set_multiplier(1+self.dk,i)
                    if re.match('[Ss]hock[ -][Tt]ube',self.fullParsedYamlFile['simulationType']):
                        temp_history=copy.deepcopy(self.run_shocktube().timeHistory)
                    elif re.match('[Rr][Cc][Mm]',self.fullParsedYamlFile['simulationType']):
                ...
                for i in range(self.gas.n_reactions):
                    ...
                    print(nominal,'THIS IS NOMINAL')
                    sens[i]=self.sensitivityCalculation(nominal,delay,self.dk)
                    print('sensitivity',i,':',sens[i])
                    self.gas.set_multiplier(1.0,i)
    ...
    def BFM_paralell(self,nominal,n_procs=5,simulation_type = ''):
        info=[]
        sens=[]
        ...
        args2 = (self.dk,self.processor.cti_path,nominal,self.target_type,self.target)
        num = self.processor.solution.n_reactions

        def run_shocktube(args=[],temp_proc=None):
        ...
        def ig_X_specific_value(data,target,value,unit=''):
        ...
        def sensitivityCalculationParalell(originalValues,newValues,dk=args2[0]):
            sensitivity=(np.log(newValues)-np.log(originalValues))/dk
                            
            return sensitivity   

        def solve_sens(i,args,args2): 
            if re.match('^[Mm]ax [Dd]erivative$',args2[3]):
                if re.match('[Pp]ressure',args2[4]):  
                    temp_processor=ctp.Processor(args2[1])
                    temp_processor.solution.set_multiplier(1+args2[0],i)
                    if re.match('[Ss]hock[ -][Tt]ube',args[11]['simulationType']):
                        temp_history=copy.deepcopy(run_shocktube(args,temp_proc=temp_processor).timeHistory)  
                        delay=ig_dPdt(temp_history) 
                        sens=sensitivityCalculationParalell(nominal,delay,args2[0])
                    elif re.match('[Rr][Cc][Mm]',args[11]['simulationType']):
                        print('This function not bult in yet')

            elif re.match('[Ss]pecific[ -][Vv]alue',args2[3]): 
                split_item = args2[4].split(' ')
                if len(split_item) ==2:
                    split_item.append('')
                
                if re.match('[Tt]emperature',split_item[0]):
                    print('Need to copy from other function')
                elif re.match('[Pp]ressure',split_item[0]):
                    print('need to copy from other function')
                elif not re.match('[Tt]emperature',split_item[0]) and not re.match('[Pp]ressure',split_item[0]):
                    temp_processor=ctp.Processor(args2[1])
                    ...
                    if re.match('[Ss]hock[ -][Tt]ube',args[11]['simulationType']):
                        temp_history=copy.deepcopy(run_shocktube(args,temp_proc=temp_processor).timeHistory)  
                        delay = ig_X_specific_value(temp_history, split_item[0], split_item[1],unit=split_item[2])
                        sens=sensitivityCalculationParalell(nominal,delay,args2[0])
        ...
        sens = Parallel(n_jobs=n_procs)(delayed(solve_sens)(i,args,args2) for i in range(num))
        sens =np.array(sens)
    ...
    def BFM_pool(self,nominal,n_procs,simulation_type = ''):
         info=[]
         sens=[]
         args=(self.pressure,self.temperature,self.observables,0,self.conditions,self.initialTime,self.finalTime,
               self.thermalBoundary,self.mechanicalBoundary,self.moleFractionObservables,self.concentrationObservables,
...
class ignition_delay_wrapper(sim.Simulation):
    ...
    def run(self):
        ...
        elif len(self.pressures) == len(self.temperatures) and len(self.pressures)>1 and len(self.temperatures)>1 and not self.volumeTraceList :
            print('T and P Itterate Together')
            for i in range(len(self.temperatures)):
                for k in range(len(self.conditions)):
                    temp_ig=ignition_delay(pressure=self.pressures[i],
                                                   temperature=self.temperatures[i],

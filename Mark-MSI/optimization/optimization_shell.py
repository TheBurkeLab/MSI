...
# pre process the cti file to remove reactions and rename it,also save it as the first run of the file
    ...
    def adding_k_target_values(self,loop_counter=0):
        ...
        k_target_values_for_S = self.adding_target_values_instance.target_values_for_S(self.data_directory+'/'+ self.k_target_values_csv,
                                                                                 ...
                                                                                 self.S_matrix,
                                                                                 master_equation_reaction_list = self.master_equation_reactions,
                                                                                 master_equation_sensitivites = None)    
    ...
    def multiple_runs(self,loops):
        ...
        for loop in range(loops):            
            ...
            physical_parameters_df = self.matrix_builder_instance.get_physical_parameters_df(self.experiment_dictonaries,
            ...
            if self.matrix_path == None:
                ...
                for i in range(len(delta_X_list)):
                    convergence[str(i)]=np.array(delta_X_list[i].sort_index()['X_values']).flatten()
                    convergence['Parameter']=self.X_data_frame['value']
                convergence.to_csv(os.path.join(self.matrix_path,'convergence_data.csv'),index=False)       

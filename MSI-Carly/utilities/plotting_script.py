...
class Plotting(object):
    def __init__(self,S_matrix,
        ...
        self.set_n_Ea_to_zero = set_n_Ea_to_zero
        self.T_min = T_min
        self.T_max = T_max
        self.P_min = P_min
        self.P_max = P_max
    ...
    def lengths_of_experimental_data(self):
        ...
        return observable_counter+absorbance_wl,simulation_lengths_of_experimental_data
    ...
    def run_ignition_delay(self,exp,cti,n_of_data_points=10):
        #fix ignition delay plotter
        p=pr.Processor(cti)
        if 'volumeTraceCsv' not in exp['simulation'].fullParsedYamlFile.keys():
            if len(exp['simulation'].fullParsedYamlFile['temperatures'])>1 and len(exp['simulation'].fullParsedYamlFile['pressures'])==1 :
            ...
            elif len(exp['simulation'].fullParsedYamlFile['pressures'])>1 and len(exp['simulation'].fullParsedYamlFile['temperatures'])==1:
            ...
            elif len(exp['simulation'].fullParsedYamlFile['pressures']) ==len(exp['simulation'].fullParsedYamlFile['temperatures']):   
                print('in here in plotter',exp['simulation'].fullParsedYamlFile['target_type'])
                tempmin=np.min(exp['simulation'].fullParsedYamlFile['temperatures'])
                tempmax=np.max(exp['simulation'].fullParsedYamlFile['temperatures'])
                total_range=tempmax-tempmin
                tempmax=tempmax+0.1*total_range
                ...
                if observable in exp['concentration_observables']:
                    ...
                    if re.match('[Ss]hock [Tt]ube',exp['simulation_type']) and re.match('[Ss]pecies[ -][Pp]rofile',exp['experiment_type']):
                        ...
                        if observable+'_ppm' in exp['experimental_data'][observable_counter].columns:
                            ...
                            plt.plot(self.exp_dict_list_original[i]['simulation'].timeHistories[0]['time']*1e3,self.exp_dict_list_original[i]['simulation'].timeHistories[0][observable]*1e6,'r',label= "$a$ $priori$ model")
                            plt.scatter(exp['experimental_data'][observable_counter]['Time']*1e3,exp['experimental_data'][observable_counter][observable+'_ppm'],marker='o',facecolors='none',color='black',label='Experimental Data') 
                            plt.xlabel('Time (ms)')
                            plt.ylabel(str(observable)+ ' mole fraction (ppm)')
                            # plt.title('Experiment_'+str(i+1))
                            ...
                            plt.plot(self.exp_dict_list_original[i]['simulation'].timeHistories[0]['time']*1e3,concentration_original,'r',label= "$a$ $priori$ model")
                            plt.scatter(exp['experimental_data'][observable_counter]['Time']*1e3,exp['experimental_data'][observable_counter][observable+'_mol/cm^3'],marker='o',facecolors='none',color='black',label='Experimental Data') 
                            plt.xlabel('Time (ms)')
                            plt.ylabel('['+str(observable)+'] ('+r'$\frac{mol}{cm^3}$'+')')
                            # plt.title('Experiment_'+str(i+1))
                        ...
                        for key in self.exp_dict_list_original[i]['simulation'].conditions.keys():
                            ...
                            plt.plot([],'w',label= 'X$_0$ '+key+': '+str(self.exp_dict_list_original[i]['simulation'].conditions[key]))
                            key_list.append(key)
                        ...
                        # plt.savefig(self.working_directory+'/'+'Exp_'+str(i+1)+'_'+str(observable)+'_'+str(self.exp_dict_list_original[i]['simulation'].temperature)+'K_'+sp+'.pdf', bbox_inches='tight')
                        # plt.savefig(self.working_directory+'/'+'Exp_'+str(i+1)+'_'+str(observable)+'_'+str(self.exp_dict_list_original[i]['simulation'].temperature)+'K_'+sp+'.svg', bbox_inches='tight',transparent=True)
                        plt.savefig(self.working_directory+'/'+'Exp_'+str(i+1)+'_'+str(observable)+'_'+str(self.exp_dict_list_original[i]['simulation'].temperature)+'K_'+sp+'.png', bbox_inches='tight',transparent=True)
                        # print(' B ')
...
#stubb
                ...
                # plt.savefig(self.working_directory+'/'+reaction_list_from_mechanism[reaction]+'.png', bbox_inches='tight')
    ...
    def sort_top_uncertainty_weighted_sens(self,top_sensitivity=10):
        ...
        short_sigma = self.shorten_sigma()
        sigma_csv = self.sigma_uncertainty_weighted_sensitivity_csv
        observable_list= self.get_observables_list()
        ...
        elif self.sigma_ones==True and self.set_n_Ea_to_zero==False:
            shape = len(self.short_sigma)
            Sig = np.ones((shape,1))

        elif self.set_n_Ea_to_zero==True and self.sigma_ones==False:
            
            shape = len(self.short_sigma)
            sig_lst = []
            for i,obs in enumerate(observable_list):
                if 'n_' in obs:
                    sig_lst.append(0)
                elif 'Ea_' in obs:
                    sig_lst.append(0)
                else:
                    sig_lst.append(short_sigma[i][0])

            sig_lst  = np.array(sig_lst)       
            sig_lst = sig_lst.reshape((sig_lst.shape[0],1)) 
            Sig=sig_lst
        
        elif self.set_n_Ea_to_zero==True and self.sigma_ones==True:
            shape = len(self.short_sigma)
            sig_lst  = []
            for i,obs in enumerate(observable_list):
                if 'n_' in obs:
    ...
    def get_observables_list(self):
        ...
        if bool(sigma_csv):
            ...
            self.observable_list_transformed = observable_list_transformed
            return observable_list_transformed
    
    
    def plotting_uncertainty_weighted_sens(self):
        LegDictL = [['Time Shift','Pressure','Temperature','A Factor: CH3COCH3 + OH <=> CH2CO + CH3 + H2O','X$_0$ CH$_3$COCH$_3$','X$_0$ TBHP','Methanol PES: CH$_3$OH Frequencies','Methanol PES: Energy Relaxation Exponential Power','Methanol PES: CH$_3$ + OH Symmetry Factor','Methanol PES: Energy Relaxation Factor'],
           ['Time Shift','Temperature','Pressure','X$_0$ CH$_3$OH','2 OH <=> H2O + O PES: TS Frequencies','Methanol PES: CH$_3$OH Frequencies','Methanol PES: CH$_3$ + OH Symmetry Factor','CH$_3$ + OH <=> CH$_2$ + H2O: A Factor','H$_2$ + OH <=> H + H$_2$O: A Factor','CH$_2$ + OH <=> CH + H$_2$O: A Factor'],
           ['Time Shift','Temperature','Methanol PES: CH$_3$OH Frequencies','Methanol PES: Energy Relaxation Exponential Power','Methanol PES: Energy Relaxation Factor','Methanol PES: Energy Relaxation Sigma','X$_0$ CH$_3$OH','Pressure','Methanol PES: CH$_3$ + OH <=> CH$_3$OH TS Energy','CH$_3$OH + OH <=> CH$_3$O + H$_2$O: A Factor'],
           ['Time Shift','Temperature','Methanol PES: Energy Relaxation Exponential Power','Methanol PES: CH$_3$OH Frequencies','Pressure','X$_0$ CH$_3$OH','Methanol PES: Energy Relaxation Factor','Methanol PES: Energy Relaxation Sigma','CH$_2$O + OH <=> H$_2$O + HCO PES: H$_2$COOH <=> H$_2$O + HCO TS Frequencies','CH$_2$O + OH <=> H$_2$O + HCO PES: CH$_2$O + OH Frequencies'],
           ['Time Shift','Temperature','Methanol PES: CH$_3$OH Frequencies','Methanol PES: Energy Relaxation Exponential Power','X$_0$ CH$_3$OH','Pressure','Methanol PES: Energy Relaxation Factor','Methanol PES: Energy Relaxation Sigma','CH$_3$OH + OH <=> CH$_3$O + H$_2$O: A Factor','Methanol PES: CH$_3$OH Symmetry Factor'],
           ['Time Shift','Temperature','Methanol PES: CH$_3$OH Frequencies','Methanol PES: Energy Relaxation Exponential Power','X$_0$ CH$_3$OH','Pressure','Methanol PES: Energy Relaxation Factor','Methanol PES: Energy Relaxation Sigma','CH$_3$OH + OH <=> CH$_3$O + H$_2$O: A Factor','Methanol PES: CH$_3$OH Symmetry Factor'],
           ['Time Shift','Temperature','Methanol PES: CH$_3$OH Frequencies','Methanol PES: Energy Relaxation Exponential Power','X$_0$ CH$_3$OH','Pressure','Methanol PES: Energy Relaxation Factor','Methanol PES: Energy Relaxation Sigma','CH$_3$OH + OH <=> CH$_3$O + H$_2$O: A Factor','Methanol PES: CH$_3$ + OH <=> CH$_3$OH TS Energy'],
           ['Time Shift','Temperature','Methanol PES: CH$_3$OH Frequencies','Methanol PES: Energy Relaxation Exponential Power','X$_0$ CH$_3$OH','Pressure','Methanol PES: Energy Relaxation Factor','Methanol PES: Energy Relaxation Sigma','CH$_3$OH + OH <=> CH$_3$O + H$_2$O: A Factor','Methanol PES: CH$_3$OH Symmetry Factor'],
           ['Time Shift','Temperature','Methanol PES: CH$_3$OH Frequencies','Methanol PES: Energy Relaxation Exponential Power','X$_0$ CH$_3$OH','Pressure','Methanol PES: Energy Relaxation Factor','Methanol PES: Energy Relaxation Sigma','CH$_3$OH + OH <=> CH$_3$O + H$_2$O: A Factor','Methanol PES: CH$_3$OH Symmetry Factor'],
           ['Time Shift','Temperature','Methanol PES: CH$_3$OH Frequencies','Methanol PES: Energy Relaxation Exponential Power','X$_0$ CH$_3$OH','Pressure','Methanol PES: Energy Relaxation Factor','Methanol PES: Energy Relaxation Sigma','Methanol PES: CH$_3$OH Symmetry Factor','Methanol PES: CH$_3$ + OH <=> CH$_3$OH TS Energy'],
           ['Time Shift','Temperature','Methanol PES: Energy Relaxation Exponential Power','Pressure','Methanol PES: CH$_3$OH Frequencies','X$_0$ CH$_3$OH','Methanol PES: Energy Relaxation Factor','X$_0$ H$_2$','Methanol PES: Energy Relaxation Sigma','H$_2$ + OH <=> H + H$_2$O: A Factor'],
           ['Time Shift','Temperature','Methanol PES: Energy Relaxation Exponential Power','Pressure','Methanol PES: CH$_3$OH Frequencies','X$_0$ CH$_3$OH','Methanol PES: Energy Relaxation Factor','X$_0$ H$_2$','Methanol PES: Energy Relaxation Sigma','H$_2$ + OH <=> H + H$_2$O: A Factor'],
           ['Time Shift','Temperature','Methanol PES: Energy Relaxation Exponential Power','Pressure','Methanol PES: CH$_3$OH Frequencies','Methanol PES: Energy Relaxation Factor','X$_0$ CH$_3$OH','Methanol PES: Energy Relaxation Sigma','X$_0$ H$_2$','H$_2$ + OH <=> H + H$_2$O: A Factor'],
           ['Time Shift','Pressure','X$_0$ C$_2$H$_6$N$_2$','X$_0$ TBHP','Methanol PES: CH$_3$OH Frequencies','Temperature','Methanol PES: CH$_3$ + OH Symmetry Factor','Methanol PES: Energy Relaxation Exponential Power','Methanol PES: Energy Relaxation Factor','Methanol PES: Energy Relaxation Sigma'],
           ['Time Shift','X$_0$ TBHP','Pressure','Temperature','A Factor: CH3COCH3 + OH <=> CH2CO + CH3 + H2O','Methanol PES: CH$_3$ + OH Symmetry Factor','Methanol PES: CH$_3$OH Frequencies','Methanol PES: Energy Relaxation Exponential Power','2 OH <=> H2O + O PES: TS Frequencies','Methanol PES: Energy Relaxation Factor'],
           ['Time Shift','Temperature','Methanol PES: Energy Relaxation Exponential Power','Methanol PES: CH$_3$OH Frequencies','Methanol PES: Energy Relaxation Factor','X$_0$ CH$_3$OH','Methanol PES: Energy Relaxation Sigma','Pressure','Methanol PES: CH$_3$ + OH <=> CH$_3$OH TS Energy','Methanol PES: CH$_3$OH Symmetry Factor']]
        LegDictS = []
        ...
        def subplot_function(number_of_observables_in_simulation,time_profiles,sensitivities,top_sensitivity_single_exp,observables_list,list_of_experiment_observables,experiment_number):
            ...
            for plot_number in range(number_of_observables_in_simulation):
                for c,top_columns in enumerate(top_sensitivity_single_exp[plot_number]):
                    print(top_columns)
                    # print(observables_list[top_columns])
                    ...
                    plt.plot(time_profiles[plot_number],sensitivities[plot_number][:,c],label = str(top_columns) +'_'+str(sigma_list[top_columns]))
                    # plt.plot(time_profiles[plot_number],sensitivities[plot_number][:,c],label = LegDictL[x][c]+'_'+str(sigma_list[top_columns]))     
                    plt.subplots_adjust(left=None, bottom=None, right=None, top=None, wspace=None, hspace=1.5)
                    plt.ylabel(list_of_experiment_observables[plot_number]+' sensitivity, $S$')
                    plt.xlabel('Time (ms)')
                    # if x == 1:
                    #     plt.ylim(-3,1)
                    # elif x == 13:
            ...
            if self.simulation_run==None:
                ...
                plt.savefig(self.working_directory+'/'+'Experiment_'+str(experiment_number+1)+'.png', bbox_inches='tight')
            else:
           ...
           #############################################
            ...
            for i,reaction in enumerate(unique_reactions_optimized):
               ...
               # print(reaction_list_from_mechanism_original.index(reaction_list_from_mechanism[reaction]))
                ...
                # plt.savefig(os.path.join(self.working_directory,reaction_list_from_mechanism[reaction]+'.png'), bbox_inches='tight')
...
#paste here
    ...
    #                # print(a,b)
                ...
                if type(reaction) != tuple:
                    if reaction_list_from_mechanism[reaction] =='H + O2 <=> O + OH':
                          df_og = pd.DataFrame()
                    ...
                    if reaction_list_from_mechanism[reaction] =='H2 + OH <=> H + H2O':
                          df_og2 = pd.DataFrame()
                    ...
                    if reaction_list_from_mechanism[reaction] =='H2 + O <=> H + OH':
                          df_og3 = pd.DataFrame()
                    ...
                    if reaction_list_from_mechanism[reaction] =='H + HO2 <=> H2 + O2':
                          df_og4 = pd.DataFrame()
                          df_og4['Temperature'] = list(Temp_optimized)
                          df_og4['k_og_optimized'] = list(k_optimized)
                          df_og4['target_vaue_temps_optimized'] = pd.Series(a)
                          df_og4['low_error_optimized'] = pd.Series(bb)
                          df_og4['high_error_optimized'] = pd.Series(b)  

                    if reaction_list_from_mechanism[reaction] =='H + HO2 <=> 2 OH':
                          df_og5 = pd.DataFrame()
                          df_og5['Temperature'] = list(Temp_optimized)
                          df_og5['k_og_optimized'] = list(k_optimized)
                          df_og5['target_vaue_temps_optimized'] = pd.Series(a)
                          df_og5['low_error_optimized'] = pd.Series(bb)
                          df_og5['high_error_optimized'] = pd.Series(b) 

                    if reaction_list_from_mechanism[reaction] =='H + HO2 <=> H2O + O':
                          df_og6 = pd.DataFrame()
                          df_og6['Temperature'] = list(Temp_optimized)
                          df_og6['k_og_optimized'] = list(k_optimized)
                          df_og6['target_vaue_temps_optimized'] = pd.Series(a)
                          df_og6['low_error_optimized'] = pd.Series(bb)
                          df_og6['high_error_optimized'] = pd.Series(b) 

                    if reaction_list_from_mechanism[reaction] =='H + O2 (+M) <=> HO2 (+M)':
                          df_og7 = pd.DataFrame()
                          df_og7['Temperature'] = list(Temp_optimized)
                          df_og7['k_og_optimized'] = list(k_optimized)
                          df_og7['target_vaue_temps_optimized'] = pd.Series(a)
                          df_og7['low_error_optimized'] = pd.Series(bb)
                          df_og7['high_error_optimized'] = pd.Series(b) 


                    if reaction_list_from_mechanism[reaction] =='H + NO2 <=> NO + OH':
                          df_og8 = pd.DataFrame()
                          df_og8['Temperature'] = list(Temp_optimized)
                          df_og8['k_og_optimized'] = list(k_optimized)
                          df_og8['target_vaue_temps_optimized'] = pd.Series(a)
                          df_og8['low_error_optimized'] = pd.Series(bb)
                          df_og8['high_error_optimized'] = pd.Series(b) 

                    if reaction_list_from_mechanism[reaction] =='CH3 + HO2 <=> CH4 + O2':
                          df_og9 = pd.DataFrame()
                          df_og9['Temperature'] = list(Temp_optimized)
                          df_og9['k_og_optimized'] = list(k_optimized)
                          df_og9['target_vaue_temps_optimized'] = pd.Series(a)
                          df_og9['low_error_optimized'] = pd.Series(bb)
                          df_og9['high_error_optimized'] = pd.Series(b) 
                    
                    if reaction_list_from_mechanism[reaction] =='CH3 + HO2 <=> CH3O + OH':
                          df_og10 = pd.DataFrame()
                          df_og10['Temperature'] = list(Temp_optimized)
                          df_og10['k_og_optimized'] = list(k_optimized)
                          df_og10['target_vaue_temps_optimized'] = pd.Series(a)
                          df_og10['low_error_optimized'] = pd.Series(bb)
                          df_og10['high_error_optimized'] = pd.Series(b) 


                    if reaction_list_from_mechanism[reaction] =='CH4 + OH <=> CH3 + H2O':
                          df_og11 = pd.DataFrame()
                          df_og11['Temperature'] = list(Temp_optimized)
                          df_og11['k_og_optimized'] = list(k_optimized)
                          df_og11['target_vaue_temps_optimized'] = pd.Series(a)
                          df_og11['low_error_optimized'] = pd.Series(bb)
                          df_og11['high_error_optimized'] = pd.Series(b) 


                    if reaction_list_from_mechanism[reaction] =='C2H6 <=> 2 CH3':
                          df_og12 = pd.DataFrame()
                          df_og12['Temperature'] = list(Temp_optimized)
                          df_og12['k_og_optimized'] = list(k_optimized)
                          df_og12['target_vaue_temps_optimized'] = pd.Series(a)
                          df_og12['low_error_optimized'] = pd.Series(bb)
                          df_og12['high_error_optimized'] = pd.Series(b) 

                    if reaction_list_from_mechanism[reaction] =='2 CH3 <=> C2H5 + H':
                          df_og13 = pd.DataFrame()
                          df_og13['Temperature'] = list(Temp_optimized)
                          df_og13['k_og_optimized'] = list(k_optimized)
                          df_og13['target_vaue_temps_optimized'] = pd.Series(a)
                          df_og13['low_error_optimized'] = pd.Series(bb)
                          df_og13['high_error_optimized'] = pd.Series(b) 

                    if reaction_list_from_mechanism[reaction] =='2 CH3 <=> C2H4 + H2':
                          df_og14 = pd.DataFrame()
                          df_og14['Temperature'] = list(Temp_optimized)
                          df_og14['k_og_optimized'] = list(k_optimized)
                          df_og14['target_vaue_temps_optimized'] = pd.Series(a)
                          df_og14['low_error_optimized'] = pd.Series(bb)
                          df_og14['high_error_optimized'] = pd.Series(b) 

                    if reaction_list_from_mechanism[reaction] =='CH3COCH3 + OH <=> CH2CO + CH3 + H2O':
                          df_og15 = pd.DataFrame()
                          df_og15['Temperature'] = list(Temp_optimized)
                          df_og15['k_og_optimized'] = list(k_optimized)
                          df_og15['target_vaue_temps_optimized'] = pd.Series(a)
                          df_og15['low_error_optimized'] = pd.Series(bb)
                          df_og15['high_error_optimized'] = pd.Series(b) 

                    if reaction_list_from_mechanism[reaction] =='CH3 + OH <=> CH2 + H2O':
                          df_og16 = pd.DataFrame()
                          df_og16['Temperature'] = list(Temp_optimized)
                          df_og16['k_og_optimized'] = list(k_optimized)
                          df_og16['target_vaue_temps_optimized'] = pd.Series(a)
                          df_og16['low_error_optimized'] = pd.Series(bb)
                          df_og16['high_error_optimized'] = pd.Series(b) 

                    if reaction_list_from_mechanism[reaction] =='CH2O <=> CO + H2':
                          df_og17 = pd.DataFrame()
                          df_og17['Temperature'] = list(Temp_optimized)
                          df_og17['k_og_optimized'] = list(k_optimized)
                          df_og17['target_vaue_temps_optimized'] = pd.Series(a)
                          df_og17['low_error_optimized'] = pd.Series(bb)
                          df_og17['high_error_optimized'] = pd.Series(b) 

                    if reaction_list_from_mechanism[reaction] =='H + HCO <=> CO + H2':
                          df_og18 = pd.DataFrame()
                          df_og18['Temperature'] = list(Temp_optimized)
                          df_og18['k_og_optimized'] = list(k_optimized)
                          df_og18['target_vaue_temps_optimized'] = pd.Series(a)
                          df_og18['low_error_optimized'] = pd.Series(bb)
                          df_og18['high_error_optimized'] = pd.Series(b) 


                    if reaction_list_from_mechanism[reaction] =='CH2O <=> H + HCO':
                          df_og19 = pd.DataFrame()
                          df_og19['Temperature'] = list(Temp_optimized)
                          df_og19['k_og_optimized'] = list(k_optimized)
                          df_og19['target_vaue_temps_optimized'] = pd.Series(a)
                          df_og19['low_error_optimized'] = pd.Series(bb)
                          df_og19['high_error_optimized'] = pd.Series(b) 

                    if reaction_list_from_mechanism[reaction] =='CH2O + O2 <=> HCO + HO2':
                          df_og20 = pd.DataFrame()
                          df_og20['Temperature'] = list(Temp_optimized)
                          df_og20['k_og_optimized'] = list(k_optimized)
                          df_og20['target_vaue_temps_optimized'] = pd.Series(a)
                          df_og20['low_error_optimized'] = pd.Series(bb)
                          df_og20['high_error_optimized'] = pd.Series(b)


                    if reaction_list_from_mechanism[reaction] =='HCO <=> CO + H':
                          df_og21 = pd.DataFrame()
                          df_og21['Temperature'] = list(Temp_optimized)
                          df_og21['k_og_optimized'] = list(k_optimized)
                          df_og21['target_vaue_temps_optimized'] = pd.Series(a)
                          df_og21['low_error_optimized'] = pd.Series(bb)
                          df_og21['high_error_optimized'] = pd.Series(b)
                
                    if reaction_list_from_mechanism[reaction] =='HCO + O2 <=> CO + HO2':
                          df_og22 = pd.DataFrame()
                          df_og22['Temperature'] = list(Temp_optimized)
                          df_og22['k_og_optimized'] = list(k_optimized)
                          df_og22['target_vaue_temps_optimized'] = pd.Series(a)
                          df_og22['low_error_optimized'] = pd.Series(bb)
                          df_og22['high_error_optimized'] = pd.Series(b)

                    if reaction_list_from_mechanism[reaction] =='CH2O + H <=> H2 + HCO-exc':
                          df_og23 = pd.DataFrame()
                          df_og23['Temperature'] = list(Temp_optimized)
                          df_og23['k_og_optimized'] = list(k_optimized)
                          df_og23['target_vaue_temps_optimized'] = pd.Series(a)
                          df_og23['low_error_optimized'] = pd.Series(bb)
                          df_og23['high_error_optimized'] = pd.Series(b)

                    if reaction_list_from_mechanism[reaction] =='CH2O + OH <=> H2O + HCO-exc2':
                          df_og24 = pd.DataFrame()
                          df_og24['Temperature'] = list(Temp_optimized)
                          df_og24['k_og_optimized'] = list(k_optimized)
                          df_og24['target_vaue_temps_optimized'] = pd.Series(a)
                          df_og24['low_error_optimized'] = pd.Series(bb)
                          df_og24['high_error_optimized'] = pd.Series(b)


                    if reaction_list_from_mechanism[reaction] =='CO + OH <=> CO2 + H':
                          df_og25 = pd.DataFrame()
                          df_og25['Temperature'] = list(Temp_optimized)
                          df_og25['k_og_optimized'] = list(k_optimized)
                          df_og25['target_vaue_temps_optimized'] = pd.Series(a)
                          df_og25['low_error_optimized'] = pd.Series(bb)
                          df_og25['high_error_optimized'] = pd.Series(b)


                    if reaction_list_from_mechanism[reaction] =='CO + OH <=> HOCO':
                          df_og26 = pd.DataFrame()
                          df_og26['Temperature'] = list(Temp_optimized)
                          df_og26['k_og_optimized'] = list(k_optimized)
                          df_og26['target_vaue_temps_optimized'] = pd.Series(a)
                          df_og26['low_error_optimized'] = pd.Series(bb)
                          df_og26['high_error_optimized'] = pd.Series(b)

                    if reaction_list_from_mechanism[reaction] =='CH3 + O2 <=> CH3O + O':
                          df_og27 = pd.DataFrame()
                          df_og27['Temperature'] = list(Temp_optimized)
                          df_og27['k_og_optimized'] = list(k_optimized)
                          df_og27['target_vaue_temps_optimized'] = pd.Series(a)
                          df_og27['low_error_optimized'] = pd.Series(bb)
                          df_og27['high_error_optimized'] = pd.Series(b)


                    if reaction_list_from_mechanism[reaction] =='CH3 + O2 <=> CH2O + OH':
                          df_og28 = pd.DataFrame()
                          df_og28['Temperature'] = list(Temp_optimized)
                          df_og28['k_og_optimized'] = list(k_optimized)
                          df_og28['target_vaue_temps_optimized'] = pd.Series(a)
                          df_og28['low_error_optimized'] = pd.Series(bb)
                          df_og28['high_error_optimized'] = pd.Series(b)
                    ...
                    if reaction_list_from_mechanism[reaction] =='CH3OH <=> CH3 + OH':
                          df_og29 = pd.DataFrame()
                          df_og29['Temperature'] = list(Temp_optimized)
                          df_og29['k_og_optimized'] = list(k_optimized)
                          df_og29['target_vaue_temps_optimized'] = pd.Series(a)
                          df_og29['low_error_optimized'] = pd.Series(bb)
                          df_og29['high_error_optimized'] = pd.Series(b)

                    if reaction_list_from_mechanism[reaction] =='CH3OH <=> CH3O + H':
                          df_og30 = pd.DataFrame()
                          df_og30['Temperature'] = list(Temp_optimized)
                          df_og30['k_og_optimized'] = list(k_optimized)
                          df_og30['target_vaue_temps_optimized'] = pd.Series(a)
                          df_og30['low_error_optimized'] = pd.Series(bb)
                          df_og30['high_error_optimized'] = pd.Series(b)

                    if reaction_list_from_mechanism[reaction] =='CH2O + H (+M) <=> CH3O (+M)':
                          df_og31 = pd.DataFrame()
                          df_og31['Temperature'] = list(Temp_optimized)
                          df_og31['k_og_optimized'] = list(k_optimized)
                          df_og31['target_vaue_temps_optimized'] = pd.Series(a)
                          df_og31['low_error_optimized'] = pd.Series(bb)
                          df_og31['high_error_optimized'] = pd.Series(b)
    ...
    # #stubb
                    ...
                    if reaction_list_from_mechanism_original[reaction_list_from_mechanism_original.index(reaction_list_from_mechanism[reaction])] =='H + O2 <=> O + OH':
                        df_og['k_original'] = k_original

                        
                    if reaction_list_from_mechanism_original[reaction_list_from_mechanism_original.index(reaction_list_from_mechanism[reaction])] =='H2 + OH <=> H + H2O':
                        df_og2['k_original'] = k_original

                    if reaction_list_from_mechanism_original[reaction_list_from_mechanism_original.index(reaction_list_from_mechanism[reaction])] =='H2 + O <=> H + OH':
                        df_og3['k_original'] = k_original 


                    if reaction_list_from_mechanism_original[reaction_list_from_mechanism_original.index(reaction_list_from_mechanism[reaction])] =='H + HO2 <=> H2 + O2':
                        df_og4['k_original'] = k_original 
                    ...
                    if reaction_list_from_mechanism_original[reaction_list_from_mechanism_original.index(reaction_list_from_mechanism[reaction])] =='H + HO2 <=> 2 OH':
                        df_og5['k_original'] = k_original 
                    
                    
                    if reaction_list_from_mechanism_original[reaction_list_from_mechanism_original.index(reaction_list_from_mechanism[reaction])] =='H + HO2 <=> H2O + O':
                        df_og6['k_original'] = k_original 

                    if reaction_list_from_mechanism_original[reaction_list_from_mechanism_original.index(reaction_list_from_mechanism[reaction])] =='H + O2 (+M) <=> HO2 (+M)':
                        df_og7['k_original'] = k_original 

                    if reaction_list_from_mechanism_original[reaction_list_from_mechanism_original.index(reaction_list_from_mechanism[reaction])] =='H + NO2 <=> NO + OH':
                        df_og8['k_original'] = k_original 

                    if reaction_list_from_mechanism_original[reaction_list_from_mechanism_original.index(reaction_list_from_mechanism[reaction])] =='CH3 + HO2 <=> CH4 + O2':
                        df_og9['k_original'] = k_original 


                    if reaction_list_from_mechanism_original[reaction_list_from_mechanism_original.index(reaction_list_from_mechanism[reaction])] =='CH3 + HO2 <=> CH3O + OH':
                        df_og10['k_original'] = k_original 

                    if reaction_list_from_mechanism_original[reaction_list_from_mechanism_original.index(reaction_list_from_mechanism[reaction])] =='CH4 + OH <=> CH3 + H2O':
                        df_og11['k_original'] = k_original 
                    
                    if reaction_list_from_mechanism_original[reaction_list_from_mechanism_original.index(reaction_list_from_mechanism[reaction])] =='C2H6 <=> 2 CH3':
                        df_og12['k_original'] = k_original  

                    if reaction_list_from_mechanism_original[reaction_list_from_mechanism_original.index(reaction_list_from_mechanism[reaction])] =='2 CH3 <=> C2H5 + H':
                        df_og13['k_original'] = k_original   


                    if reaction_list_from_mechanism_original[reaction_list_from_mechanism_original.index(reaction_list_from_mechanism[reaction])] =='2 CH3 <=> C2H4 + H2':
                        df_og14['k_original'] = k_original   

                    if reaction_list_from_mechanism_original[reaction_list_from_mechanism_original.index(reaction_list_from_mechanism[reaction])] =='CH3COCH3 + OH <=> CH2CO + CH3 + H2O':
                        df_og15['k_original'] = k_original                 

                    if reaction_list_from_mechanism_original[reaction_list_from_mechanism_original.index(reaction_list_from_mechanism[reaction])] =='CH3 + OH <=> CH2 + H2O':
                        df_og16['k_original'] = k_original         


                    if reaction_list_from_mechanism_original[reaction_list_from_mechanism_original.index(reaction_list_from_mechanism[reaction])] =='CH2O <=> CO + H2':
                        df_og17['k_original'] = k_original                                     
               

                    if reaction_list_from_mechanism_original[reaction_list_from_mechanism_original.index(reaction_list_from_mechanism[reaction])] =='H + HCO <=> CO + H2':
                        df_og18['k_original'] = k_original  

                    if reaction_list_from_mechanism_original[reaction_list_from_mechanism_original.index(reaction_list_from_mechanism[reaction])] =='CH2O <=> H + HCO':
                        df_og19['k_original'] = k_original 

                    if reaction_list_from_mechanism_original[reaction_list_from_mechanism_original.index(reaction_list_from_mechanism[reaction])] =='CH2O + O2 <=> HCO + HO2':
                        df_og20['k_original'] = k_original 


                    if reaction_list_from_mechanism_original[reaction_list_from_mechanism_original.index(reaction_list_from_mechanism[reaction])] =='HCO <=> CO + H':
                        df_og21['k_original'] = k_original 


                    if reaction_list_from_mechanism_original[reaction_list_from_mechanism_original.index(reaction_list_from_mechanism[reaction])] =='HCO + O2 <=> CO + HO2':
                        df_og22['k_original'] = k_original 

                    if reaction_list_from_mechanism_original[reaction_list_from_mechanism_original.index(reaction_list_from_mechanism[reaction])] =='CH2O + H <=> H2 + HCO-exc':
                        df_og23['k_original'] = k_original                 
                
                    if reaction_list_from_mechanism_original[reaction_list_from_mechanism_original.index(reaction_list_from_mechanism[reaction])] =='CH2O + OH <=> H2O + HCO-exc2':
                        df_og24['k_original'] = k_original                  
                
                    if reaction_list_from_mechanism_original[reaction_list_from_mechanism_original.index(reaction_list_from_mechanism[reaction])] =='CO + OH <=> CO2 + H':
                        df_og25['k_original'] = k_original                 
                
                    if reaction_list_from_mechanism_original[reaction_list_from_mechanism_original.index(reaction_list_from_mechanism[reaction])] =='CO + OH <=> HOCO':
                        df_og26['k_original'] = k_original                


                    if reaction_list_from_mechanism_original[reaction_list_from_mechanism_original.index(reaction_list_from_mechanism[reaction])] =='CH3 + O2 <=> CH3O + O':
                        df_og27['k_original'] = k_original  

                    if reaction_list_from_mechanism_original[reaction_list_from_mechanism_original.index(reaction_list_from_mechanism[reaction])] =='CH3 + O2 <=> CH2O + OH':
                        df_og28['k_original'] = k_original  
                    
                    if reaction_list_from_mechanism_original[reaction_list_from_mechanism_original.index(reaction_list_from_mechanism[reaction])] =='CH3OH <=> CH3 + OH':
                        df_og29['k_original'] = k_original  

                    if reaction_list_from_mechanism_original[reaction_list_from_mechanism_original.index(reaction_list_from_mechanism[reaction])] =='CH3OH <=> CH3O + H':
                        df_og30['k_original'] = k_original  

                    if reaction_list_from_mechanism_original[reaction_list_from_mechanism_original.index(reaction_list_from_mechanism[reaction])] ==' CH2O + H (+M) <=> CH3O (+M)':
                        df_og31['k_original'] = k_original 
                   ...
                   # print(high_error_original/ target_value_ks_calculated_with_cantera_original[unique_reactions_original.index(reaction_list_from_mechanism_original.index(reaction_list_from_mechanism[reaction]))])
                    ...
                    if type(reaction) != tuple:
                        if reaction_list_from_mechanism[reaction] =='H + O2 <=> O + OH':
                              ...
                              df_og.to_csv('/home/jmp/MSIFormaldehyde_CarlyOct23/MSI/data/branching_reaction_study/rate_constant_csv_files_for_plotting/H + O2 <=> O + OH_k_optimized.csv')
                        ...
                        if reaction_list_from_mechanism[reaction] =='H2 + OH <=> H + H2O':
                              ...
                              df_og2.to_csv('/home/jmp/MSIFormaldehyde_CarlyOct23/MSI/data/branching_reaction_study/rate_constant_csv_files_for_plotting/H2 + OH <=> H + H2O_k_optimized.csv')
                        ...
                        if reaction_list_from_mechanism[reaction] =='H2 + O <=> H + OH':
                              ...
                              df_og3.to_csv('/home/jmp/MSIFormaldehyde_CarlyOct23/MSI/data/branching_reaction_study/rate_constant_csv_files_for_plotting/H2 + O <=> H + OH_k_optimized.csv')
                       
                        if reaction_list_from_mechanism[reaction] =='H + HO2 <=> H2 + O2':
                              df_og4['target_vaue_temps_original'] = pd.Series(c)
                              df_og4['low_error_original'] = pd.Series(d)
                              df_og4['high_error_original'] = pd.Series(dd)

                              df_og4.to_csv('/home/jmp/MSIFormaldehyde_CarlyOct23/MSI/data/branching_reaction_study/rate_constant_csv_files_for_plotting/H + HO2 <=> H2 + O2_k_optimized.csv')

                        if reaction_list_from_mechanism[reaction] =='H + HO2 <=> 2 OH':
                              df_og5['target_vaue_temps_original'] = pd.Series(c)
                              df_og5['low_error_original'] = pd.Series(d)
                              df_og5['high_error_original'] = pd.Series(dd)

                              df_og5.to_csv('/home/jmp/MSIFormaldehyde_CarlyOct23/MSI/data/branching_reaction_study/rate_constant_csv_files_for_plotting/H + HO2 <=> 2 OH_k_optimized.csv')


                        if reaction_list_from_mechanism[reaction] =='H + HO2 <=> H2O + O':
                              df_og6['target_vaue_temps_original'] = pd.Series(c)
                              df_og6['low_error_original'] = pd.Series(d)
                              df_og6['high_error_original'] = pd.Series(dd)

                              df_og6.to_csv('/home/jmp/MSIFormaldehyde_CarlyOct23/MSI/data/branching_reaction_study/rate_constant_csv_files_for_plotting/H + HO2 <=> H2O + O_k_optimized.csv')  


                        if reaction_list_from_mechanism[reaction] =='H + O2 (+M) <=> HO2 (+M)':
                              df_og7['target_vaue_temps_original'] = pd.Series(c)
                              df_og7['low_error_original'] = pd.Series(d)
                              df_og7['high_error_original'] = pd.Series(dd)

                              df_og7.to_csv('/home/jmp/MSIFormaldehyde_CarlyOct23/MSI/data/branching_reaction_study/rate_constant_csv_files_for_plotting/H + O2 (+M) <=> HO2 (+M)_k_optimized.csv')                              
                        
                        if reaction_list_from_mechanism[reaction] =='H + NO2 <=> NO + OH':
                              df_og8['target_vaue_temps_original'] = pd.Series(c)
                              df_og8['low_error_original'] = pd.Series(d)
                              df_og8['high_error_original'] = pd.Series(dd)

                              df_og8.to_csv('/home/jmp/MSIFormaldehyde_CarlyOct23/MSI/data/branching_reaction_study/rate_constant_csv_files_for_plotting/H + NO2 <=> NO + OH_k_optimized.csv')                              

                        if reaction_list_from_mechanism[reaction] =='CH3 + HO2 <=> CH4 + O2':
                            df_og9['target_vaue_temps_original'] = pd.Series(c)
                            df_og9['low_error_original'] = pd.Series(d)
                            df_og9['high_error_original'] = pd.Series(dd)

                            df_og9.to_csv('/home/jmp/MSIFormaldehyde_CarlyOct23/MSI/data/branching_reaction_study/rate_constant_csv_files_for_plotting/CH3 + HO2 <=> CH4 + O2_k_optimized.csv')


                        if reaction_list_from_mechanism[reaction] =='CH3 + HO2 <=> CH3O + OH':
                            df_og10['target_vaue_temps_original'] = pd.Series(c)
                            df_og10['low_error_original'] = pd.Series(d)
                            df_og10['high_error_original'] = pd.Series(dd)

                            df_og10.to_csv('/home/jmp/MSIFormaldehyde_CarlyOct23/MSI/data/branching_reaction_study/rate_constant_csv_files_for_plotting/CH3 + HO2 <=> CH3O + OH_k_optimized.csv')                                


                        if reaction_list_from_mechanism[reaction] =='CH4 + OH <=> CH3 + H2O':
                            df_og11['target_vaue_temps_original'] = pd.Series(c)
                            df_og11['low_error_original'] = pd.Series(d)
                            df_og11['high_error_original'] = pd.Series(dd)

                            df_og11.to_csv('/home/jmp/MSIFormaldehyde_CarlyOct23/MSI/data/branching_reaction_study/rate_constant_csv_files_for_plotting/CH4 + OH <=> CH3 + H2O_k_optimized.csv')  


                        if reaction_list_from_mechanism[reaction] =='C2H6 <=> 2 CH3':
                            df_og12['target_vaue_temps_original'] = pd.Series(c)
                            df_og12['low_error_original'] = pd.Series(d)
                            df_og12['high_error_original'] = pd.Series(dd)

                            df_og12.to_csv('/home/jmp/MSIFormaldehyde_CarlyOct23/MSI/data/branching_reaction_study/rate_constant_csv_files_for_plotting/C2H6 <=> 2 CH3_k_optimized.csv')  


                        if reaction_list_from_mechanism[reaction] =='2 CH3 <=> C2H5 + H':
                            df_og13['target_vaue_temps_original'] = pd.Series(c)
                            df_og13['low_error_original'] = pd.Series(d)
                            df_og13['high_error_original'] = pd.Series(dd)

                            df_og13.to_csv('/home/jmp/MSIFormaldehyde_CarlyOct23/MSI/data/branching_reaction_study/rate_constant_csv_files_for_plotting/2 CH3 <=> C2H5 + H_k_optimized.csv')  

                        if reaction_list_from_mechanism[reaction] =='2 CH3 <=> C2H4 + H2':
                            df_og14['target_vaue_temps_original'] = pd.Series(c)
                            df_og14['low_error_original'] = pd.Series(d)
                            df_og14['high_error_original'] = pd.Series(dd)

                            df_og14.to_csv('/home/jmp/MSIFormaldehyde_CarlyOct23/MSI/data/branching_reaction_study/rate_constant_csv_files_for_plotting/2 CH3 <=> C2H4 + H2_k_optimized.csv')  


                        if reaction_list_from_mechanism[reaction] =='CH3COCH3 + OH <=> CH2CO + CH3 + H2O':
                            df_og15['target_vaue_temps_original'] = pd.Series(c)
                            df_og15['low_error_original'] = pd.Series(d)
                            df_og15['high_error_original'] = pd.Series(dd)

                            df_og15.to_csv('/home/jmp/MSIFormaldehyde_CarlyOct23/MSI/data/branching_reaction_study/rate_constant_csv_files_for_plotting/CH3COCH3 + OH <=> CH2CO + CH3 + H2O_k_optimized.csv')  



                        if reaction_list_from_mechanism[reaction] =='CH3 + OH <=> CH2 + H2O':
                            df_og16['target_vaue_temps_original'] = pd.Series(c)
                            df_og16['low_error_original'] = pd.Series(d)
                            df_og16['high_error_original'] = pd.Series(dd)

                            df_og16.to_csv('/home/jmp/MSIFormaldehyde_CarlyOct23/MSI/data/branching_reaction_study/rate_constant_csv_files_for_plotting/CH3 + OH <=> CH2 + H2O_k_optimized.csv')  


                        if reaction_list_from_mechanism[reaction] =='CH2O <=> CO + H2':
                            df_og17['target_vaue_temps_original'] = pd.Series(c)
                            df_og17['low_error_original'] = pd.Series(d)
                            df_og17['high_error_original'] = pd.Series(dd)

                            df_og17.to_csv('/home/jmp/MSIFormaldehyde_CarlyOct23/MSI/data/branching_reaction_study/rate_constant_csv_files_for_plotting/CH2O <=> CO + H2_k_optimized.csv')  


                        if reaction_list_from_mechanism[reaction] =='H + HCO <=> CO + H2':
                            df_og18['target_vaue_temps_original'] = pd.Series(c)
                            df_og18['low_error_original'] = pd.Series(d)
                            df_og18['high_error_original'] = pd.Series(dd)

                            df_og18.to_csv('/home/jmp/MSIFormaldehyde_CarlyOct23/MSI/data/branching_reaction_study/rate_constant_csv_files_for_plotting/H + HCO <=> CO + H2_k_optimized.csv')  



                        if reaction_list_from_mechanism[reaction] =='CH2O <=> H + HCO':
                            df_og19['target_vaue_temps_original'] = pd.Series(c)
                            df_og19['low_error_original'] = pd.Series(d)
                            df_og19['high_error_original'] = pd.Series(dd)

                            df_og19.to_csv('/home/jmp/MSIFormaldehyde_CarlyOct23/MSI/data/branching_reaction_study/rate_constant_csv_files_for_plotting/CH2O <=> H + HCO_k_optimized.csv')  

                        if reaction_list_from_mechanism[reaction] =='CH2O + O2 <=> HCO + HO2':
                            df_og20['target_vaue_temps_original'] = pd.Series(c)
                            df_og20['low_error_original'] = pd.Series(d)
                            df_og20['high_error_original'] = pd.Series(dd)

                            df_og20.to_csv('/home/jmp/MSIFormaldehyde_CarlyOct23/MSI/data/branching_reaction_study/rate_constant_csv_files_for_plotting/CH2O + O2 <=> HCO + HO2_k_optimized.csv')  


                        if reaction_list_from_mechanism[reaction] =='HCO <=> CO + H':
                            df_og21['target_vaue_temps_original'] = pd.Series(c)
                            df_og21['low_error_original'] = pd.Series(d)
                            df_og21['high_error_original'] = pd.Series(dd)

                            df_og21.to_csv('/home/jmp/MSIFormaldehyde_CarlyOct23/MSI/data/branching_reaction_study/rate_constant_csv_files_for_plotting/HCO <=> CO + H_k_optimized.csv')  

                        if reaction_list_from_mechanism[reaction] =='HCO + O2 <=> CO + HO2':
                            df_og22['target_vaue_temps_original'] = pd.Series(c)
                            df_og22['low_error_original'] = pd.Series(d)
                            df_og22['high_error_original'] = pd.Series(dd)

                            df_og22.to_csv('/home/jmp/MSIFormaldehyde_CarlyOct23/MSI/data/branching_reaction_study/rate_constant_csv_files_for_plotting/HCO + O2 <=> CO + HO2_k_optimized.csv')  

                        if reaction_list_from_mechanism[reaction] =='CH2O + H <=> H2 + HCO-exc':
                            df_og23['target_vaue_temps_original'] = pd.Series(c)
                            df_og23['low_error_original'] = pd.Series(d)
                            df_og23['high_error_original'] = pd.Series(dd)

                            df_og23.to_csv('/home/jmp/MSIFormaldehyde_CarlyOct23/MSI/data/branching_reaction_study/rate_constant_csv_files_for_plotting/CH2O + H <=> H2 + HCO-exc_k_optimized.csv')  


                        if reaction_list_from_mechanism[reaction] =='CH2O + OH <=> H2O + HCO-exc2':
                            df_og24['target_vaue_temps_original'] = pd.Series(c)
                            df_og24['low_error_original'] = pd.Series(d)
                            df_og24['high_error_original'] = pd.Series(dd)

                            df_og24.to_csv('/home/jmp/MSIFormaldehyde_CarlyOct23/MSI/data/branching_reaction_study/rate_constant_csv_files_for_plotting/CH2O + OH <=> H2O + HCO-exc2_k_optimized.csv')  

                        if reaction_list_from_mechanism[reaction] =='CO + OH <=> CO2 + H':
                            df_og25['target_vaue_temps_original'] = pd.Series(c)
                            df_og25['low_error_original'] = pd.Series(d)
                            df_og25['high_error_original'] = pd.Series(dd)

                            df_og25.to_csv('/home/jmp/MSIFormaldehyde_CarlyOct23/MSI/data/branching_reaction_study/rate_constant_csv_files_for_plotting/CO + OH <=> CO2 + H_k_optimized.csv')  



                        if reaction_list_from_mechanism[reaction] =='CO + OH <=> HOCO':
                            df_og26['target_vaue_temps_original'] = pd.Series(c)
                            df_og26['low_error_original'] = pd.Series(d)
                            df_og26['high_error_original'] = pd.Series(dd)

                            df_og26.to_csv('/home/jmp/MSIFormaldehyde_CarlyOct23/MSI/data/branching_reaction_study/rate_constant_csv_files_for_plotting/CO + OH <=> HOCO_k_optimized.csv') 


                        if reaction_list_from_mechanism[reaction] =='CH3 + O2 <=> CH3O + O':
                            df_og27['target_vaue_temps_original'] = pd.Series(c)
                            df_og27['low_error_original'] = pd.Series(d)
                            df_og27['high_error_original'] = pd.Series(dd)

                            df_og27.to_csv('/home/jmp/MSIFormaldehyde_CarlyOct23/MSI/data/branching_reaction_study/rate_constant_csv_files_for_plotting/CH3 + O2 <=> CH3O + O_k_optimized.csv')                               


                        if reaction_list_from_mechanism[reaction] =='CH3 + O2 <=> CH2O + OH':
                            df_og28['target_vaue_temps_original'] = pd.Series(c)
                            df_og28['low_error_original'] = pd.Series(d)
                            df_og28['high_error_original'] = pd.Series(dd)

                            df_og28.to_csv('/home/jmp/MSIFormaldehyde_CarlyOct23/MSI/data/branching_reaction_study/rate_constant_csv_files_for_plotting/CH3 + O2 <=> CH2O + OH_k_optimized.csv')                               

                        if reaction_list_from_mechanism[reaction] =='CH3OH <=> CH3 + OH':
                            df_og29['target_vaue_temps_original'] = pd.Series(c)
                            df_og29['low_error_original'] = pd.Series(d)
                            df_og29['high_error_original'] = pd.Series(dd)

                            df_og29.to_csv('/home/jmp/MSIFormaldehyde_CarlyOct23/MSI/data/branching_reaction_study/rate_constant_csv_files_for_plotting/CH3OH <=> CH3 + OH_k_optimized.csv')      

                        if reaction_list_from_mechanism[reaction] =='CH3OH <=> CH3O + H':
                            df_og30['target_vaue_temps_original'] = pd.Series(c)
                            df_og30['low_error_original'] = pd.Series(d)
                            df_og30['high_error_original'] = pd.Series(dd)

                            df_og30.to_csv('/home/jmp/MSIFormaldehyde_CarlyOct23/MSI/data/branching_reaction_study/rate_constant_csv_files_for_plotting/CH3OH <=> CH3O + H_k_optimized.csv')      


                        if reaction_list_from_mechanism[reaction] =='CH2O + H (+M) <=> CH3O (+M)':
                            df_og31['target_vaue_temps_original'] = pd.Series(c)
                            df_og31['low_error_original'] = pd.Series(d)
                            df_og31['high_error_original'] = pd.Series(dd)

                            df_og31.to_csv('/home/jmp/MSIFormaldehyde_CarlyOct23/MSI/data/branching_reaction_study/rate_constant_csv_files_for_plotting/CH2O + H (+M) <=> CH3O (+M)_k_optimized.csv')      
                ...
                if type(reaction)==tuple:
                    if reaction==(309, 310):
                        ...
                        # plt.savefig(self.working_directory+'/'+ttl_reaction_string[0]+'_+_'+ttl_reaction_string[1]+'.png', bbox_inches='tight')                 
                        ...
                        # plt.savefig(self.working_directory+'/'+reactants+'.png', bbox_inches='tight') 

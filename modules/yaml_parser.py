...
class Parser(object):
    ...
    def parse_flow_reactor_obj(self, loaded_exp:dict={}, loaded_absorption:dict={}):
        ...
        pressure_relative_uncertainty = float(pressure_relative_uncertainty)
        restime_relative_uncertainty = loaded_exp['apparatus']['residence-time']['relative-uncertainty']
        restime_relative_uncertainty = float(restime_relative_uncertainty)        
        ...
        for yaml_file in range(len(file_name_list)):
            ...
            if experiment_dict_list[0]['simulation'].physicalSens ==1 :
                if re.match('[Ss]hock [Tt]ube',self.original_experimental_conditions[yaml_file]['simulationType']) and re.match('[Ss]pecies[- ][Pp]rofile',self.original_experimental_conditions[yaml_file]['experimentType']):
                    ...
                    temp = self.original_experimental_conditions[yaml_file]['temperatures']
                    press = self.original_experimental_conditions[yaml_file]['pressure']
                    res = self.original_experimental_conditions[yaml_file]['residenceTimes']
                    conditions = self.original_experimental_conditions[yaml_file]['conditions']
                ...
                # print('Iteration Number: ',loop_counter+1)
                # print(temp)
                # print(press)
                # print(conditions)
                ...
                if re.match('[Ss]hock [Tt]ube',self.original_experimental_conditions[yaml_file]['simulationType']) and re.match('[Ss]pecies[- ][Pp]rofile',self.original_experimental_conditions[yaml_file]['experimentType']):
                    ...
                    self.optimized_physical_parameters.append(updatedTemp)
                    ...
                    self.optimized_physical_parameters.append(updatedPress)
                    ...
                    self.optimized_physical_parameters.append(updatedTimeShift)              
                    ...
                    for i,T in enumerate(temp):
                        ...
                        self.optimized_physical_parameters.append(float(round(np.exp(physical_observables_updates_list[yaml_file]['T'+str(i+1)+'_experiment_'+str(yaml_file)]) * T,9)))
                    ...
                    self.optimized_physical_parameters.append(updatedResTime)
                    ...
                    for i,time in enumerate(time_shifts):
                        ...
                        self.optimized_physical_parameters.append(float(round((physical_observables_updates_list[yaml_file]['Time_Shift'+str(i+1)+'_experiment_'+str(yaml_file)] + time),9)))                                   

                    updatedResTime=[]
                    for i,tau in enumerate(res):
                        updatedResTime.append(float(round(np.exp(physical_observables_updates_list[yaml_file]['R'+str(i+1)+'_experiment_'+str(yaml_file)]) * tau,9)))        
                        self.optimized_physical_parameters.append(float(round(np.exp(physical_observables_updates_list[yaml_file]['R'+str(i+1)+'_experiment_'+str(yaml_file)]) * tau,9)))        
 ...
 # starting to do file updates here 
                ...
                if re.match('[Ss]hock [Tt]ube',self.original_experimental_conditions[yaml_file]['simulationType']) and re.match('[Ss]pecies[- ][Pp]rofile',self.original_experimental_conditions[yaml_file]['experimentType']):
                    ...
                    config2['common-properties']['temperature']['value-list']=updatedTemp
                    config2['common-properties']['pressure']['value']=float(updatedPress)
                    config2['apparatus']['residence-time']['value-list']=updatedResTime
        ...
        for yaml_file in range(len(file_name_list)):
            ...
            for x in range(len(parameterOnesUpdated)):
                for y in range(len(parameterOnesUpdated[x])):
                    ...
                    self.optimized_absorption_parameters.append(round(parameterOnesUpdated[x][y], 8))
            ...
            for x in range(len(parameterTwosUpdated)):
                for y in range(len(parameterTwosUpdated[x])):
                    ...
                    if round(parameterTwosUpdated[x][y], 8) != 0:
                        self.optimized_absorption_parameters.append(round(parameterTwosUpdated[x][y], 8))

...
class Plotting(object):
    def __init__(self,
                  ...
                  P_min=1013.25,
                  P_max=1.013e+6,
                  plot_original_error = True,
                  shade_optimized_error = True,
                  shade_steps = 100,
                  shade_sigma = 2,
        ...
        self.working_directory = working_directory + '/out'
        ...
        self.P_min = P_min
        self.P_max = P_max
        self.plot_original_error = plot_original_error
        self.shade_optimized_error = shade_optimized_error
        self.shade_steps = shade_steps 
        self.shade_sigma = shade_sigma   
        self.theory_parameters_df = theory_parameters_df       
        self.T_P_min_max_dict = T_dict
        self.observable_list_for_legend = observable_list_for_legend
        self.pdf = pdf
        self.png = png
        self.svg = svg
        self.dpi = dpi
            ...
            for j,observable in enumerate(exp['mole_fraction_observables'] + exp['concentration_observables'] + exp['ignition_delay_observables']):
                ...
                if observable in exp['mole_fraction_observables']:
                    ...
                    if re.match('[Ss]hock [Tt]ube',exp['simulation_type']) and re.match('[Ss]pecies[ -][Pp]rofile',exp['experiment_type']):
                        ...
                        if bool(sigmas_optimized) == True:
                            ...
                            if self.plot_original_error == True:
                            ...
                            if len(data_df) != len(unweighted_df):
                                ...
                                if self.shade_optimized_error == True:
                                
                                    for k in range(1,self.shade_steps+1):
                                        top = exp['simulation'].timeHistoryInterpToExperiment[observable].dropna().values + (high_error_optimized-exp['simulation'].timeHistoryInterpToExperiment[observable].dropna().values)/self.shade_steps*k
                                        bottom = exp['simulation'].timeHistoryInterpToExperiment[observable].dropna().values - (exp['simulation'].timeHistoryInterpToExperiment[observable].dropna().values-low_error_optimized)/self.shade_steps*k
                                        plt.fill_between(exp['experimental_data'][observable_counter]['Time']*1e3, bottom, top, color='gray', alpha=1/self.shade_steps)      
                                    
                                # uncertainties = np.sqrt(exp['simulation'].timeHistoryInterpToExperiment[observable].dropna().values)
                                    
                                # for k in range(1,self.shade_steps+1):
                                #     top = exp['simulation'].timeHistoryInterpToExperiment[observable].dropna().values + uncertainties/self.shade_steps*k*self.shade_sigma
                                #     bottom = exp['simulation'].timeHistoryInterpToExperiment[observable].dropna().values - uncertainties/self.shade_steps*k*self.shade_sigma
                                #     plt.fill_between(exp['experimental_data'][observable_counter]['Time']*1e3, bottom, top, color='gray', alpha=1/self.shade_steps)                                                              
                        ...
                        # plt.savefig(self.working_directory+'/'+'Experiment_'+str(i+1)+'_'+str(observable)+'_'+str(self.exp_dict_list_original[i]['simulation'].temperature)+'K'+'_'+str(self.exp_dict_list_original[i]['simulation'].pressure)+'_'+sp+'_'+'.svg', bbox_inches='tight',transparent=True)
                        # temp_df_for_species.to_csv(self.working_directory+'/'+'Experiment_'+str(i+1)+'_'+str(observable)+'_'+str(self.exp_dict_list_original[i]['simulation'].temperature)+'K'+'_'+str(self.exp_dict_list_original[i]['simulation'].pressure)+'_'+sp+'.csv',index=False)  

                        
                        if self.pdf == True:
                            if self.dpi == None:
                                plt.savefig(self.working_directory+'/'+'Experiment_'+str(i+1)+'_'+str(observable)+'.pdf', bbox_inches='tight')
                            else:
                                plt.savefig(self.working_directory+'/'+'Experiment_'+str(i+1)+'_'+str(observable)+'.pdf', bbox_inches='tight',dpi=1000)
                        if self.png == True:
                            if self.dpi == None:                        
                                plt.savefig(self.working_directory+'/'+'Experiment_'+str(i+1)+'_'+str(observable)+'.png', bbox_inches='tight')
                            else:
                                plt.savefig(self.working_directory+'/'+'Experiment_'+str(i+1)+'_'+str(observable)+'.png', bbox_inches='tight',dpi=1000)
                        if self.svg == True:
                            if self.dpi == None:                                    
                                plt.savefig(self.working_directory+'/'+'Experiment_'+str(i+1)+'_'+str(observable)+'.svg', bbox_inches='tight',transparent=True)
                            else:
                                plt.savefig(self.working_directory+'/'+'Experiment_'+str(i+1)+'_'+str(observable)+'.svg', bbox_inches='tight',dpi=1000,transparent=True)
                                
                        temp_optimized_df_for_species.to_csv(self.working_directory+'/'+'Experiment_'+str(i+1)+'_'+str(observable)+'_optimized.csv',index=False)  
                        temp_optimized_error_df_for_species.to_csv(self.working_directory+'/'+'Experiment_'+str(i+1)+'_'+str(observable)+'_error_optimized.csv',index=False)  
                        temp_original_df_for_species.to_csv(self.working_directory+'/'+'Experiment_'+str(i+1)+'_'+str(observable)+'_original.csv',index=False)  
                        temp_original_error_df_for_species.to_csv(self.working_directory+'/'+'Experiment_'+str(i+1)+'_'+str(observable)+'_error_original.csv',index=False)  
                        ...
                        if bool(sigmas_optimized) == True:
                            ...
                            if self.plot_original_error == True:
                                ...
                                if len(high_error_optimized)>1 and len(low_error_optimized) > 1:
                                    pass
                                    # plt.plot(exp['experimental_data'][observable_counter]['Temperature'], high_error_original,'r--')
                                    # plt.plot(exp['experimental_data'][observable_counter]['Temperature'], low_error_original,'r--')
                                else:
                                    plt.plot(exp['experimental_data'][observable_counter]['Temperature'], high_error_original,'rx')
                                    plt.plot(exp['experimental_data'][observable_counter]['Temperature'], low_error_original,'rx')      
                                ...
                                if len(high_error_optimized)>1 and len(low_error_optimized) > 1:
                                    ...
                                    if self.shade_optimized_error == True:
                                        ...
                                        for k in range(1,self.shade_steps+1):
                                            ...
                                            top = exp['simulation'].timeHistories[0][observable]* + (high_error_optimized-exp['simulation'].timeHistories[0][observable])/self.shade_steps*k
                                            bottom = exp['simulation'].timeHistories[0][observable] - (exp['simulation'].timeHistories[0][observable]-low_error_optimized)/self.shade_steps*k
                                            plt.fill_between(exp['experimental_data'][observable_counter]['Temperature'], bottom, top, color='gray', alpha=1/self.shade_steps)    
                                                                                        
                                            # top = MSI_model[observable] + (high_error_optimized-MSI_model[observable])/self.shade_steps*k
                                            # bottom = MSI_model[observable] - (MSI_model[observable]-low_error_optimized)/self.shade_steps*k
                                            # plt.fill_between(MSI_model['temperature'], bottom, top, color='gray', alpha=1/self.shade_steps)                                   
                        ...
                        # temp_df_for_species.to_csv(self.working_directory+'/'+'Experiment_'+str(i+1)+'_'+str(observable)+'_'+str(self.exp_dict_list_original[i]['simulation'].temperature)+'K_'+sp+'.csv',index=False)    
                ...
                if observable in exp['concentration_observables']:
                    ...
                    if re.match('[Ss]hock [Tt]ube',exp['simulation_type']) and re.match('[Ss]pecies[ -][Pp]rofile',exp['experiment_type']):
                        ...
                        if observable+'_ppm' in exp['experimental_data'][observable_counter].columns:
                            ...
                            if bool(sigmas_optimized)==True:
                                ...
                                if self.shade_optimized_error == True:
                                    ...
                                    for k in range(1,self.shade_steps+1):
                                        top = exp['simulation'].timeHistoryInterpToExperiment[observable].dropna().values*1e6 + (high_error_optimized-exp['simulation'].timeHistoryInterpToExperiment[observable].dropna().values*1e6)/self.shade_steps*k
                                        bottom = exp['simulation'].timeHistoryInterpToExperiment[observable].dropna().values*1e6 - (exp['simulation'].timeHistoryInterpToExperiment[observable].dropna().values*1e6-low_error_optimized)/self.shade_steps*k
                                        plt.fill_between(exp['experimental_data'][observable_counter]['Time']*1e3, bottom, top, color='gray', alpha=1/self.shade_steps)                                               
                            ...
                            if bool(sigmas_optimized)==True:
                                ...
                                if self.shade_optimized_error == True:
                                    ...
                                    for k in range(1,self.shade_steps+1):
                                        top = concentration_optimized + (high_error_optimized-concentration_optimized)/self.shade_steps*k
                                        bottom = concentration_optimized - (concentration_optimized-low_error_optimized)/self.shade_steps*k
                                        plt.fill_between(exp['experimental_data'][observable_counter]['Time']*1e3, bottom, top, color='gray', alpha=1/self.shade_steps)    
                        ...
                        #stub
                        # plt.savefig(self.working_directory+'/'+'Experiment_'+str(i+1)+'_'+str(observable)+'_'+str(self.exp_dict_list_original[i]['simulation'].temperature)+'K_'+sp+'.pdf', bbox_inches='tight')
                        # plt.savefig(self.working_directory+'/'+'Experiment_'+str(i+1)+'_'+str(observable)+'_'+str(self.exp_dict_list_original[i]['simulation'].temperature)+'K_'+sp+'.svg', bbox_inches='tight',transparent=True)
                            ...
                            if len(data_df) != len(unweighted_df):
                                ...
                                if len(high_error_optimized)>1 and len(low_error_optimized) > 1:
                                    ...
                                    if self.shade_optimized_error == True:
                                        ...
                                        for k in range(1,self.shade_steps+1):
                                            top = exp['simulation'].timeHistories[0][observable]*1e6 + (high_error_optimized-exp['simulation'].timeHistories[0][observable]*1e6)/self.shade_steps*k
                                            bottom = exp['simulation'].timeHistories[0][observable]*1e6 - (exp['simulation'].timeHistories[0][observable]*1e6-low_error_optimized)/self.shade_steps*k
                                            plt.fill_between(exp['experimental_data'][observable_counter]['Temperature'], bottom, top, color='gray', alpha=1/self.shade_steps)        
                        ...
                        # 
                        # 
                        # plt.savefig(self.working_directory+'/'+'Experiment_'+str(i+1)+'_'+str(observable)+'_'+str(self.exp_dict_list_original[i]['simulation'].temperature)+'K_'+sp+'.pdf', bbox_inches='tight',dpi=1000) 
                        # plt.savefig(self.working_directory+'/'+'Experiment_'+str(i+1)+'_'+str(observable)+'_'+str(self.exp_dict_list_original[i]['simulation'].temperature)+'K_'+sp+'.svg', bbox_inches='tight',dpi=1000,transparent=True)             
                        # temp_df_for_species.to_csv(self.working_directory+'/'+'Experiment_'+str(i+1)+'_'+str(observable)+'_'+str(self.exp_dict_list_original[i]['simulation'].temperature)+'K_'+sp+'.csv',index=False)                            
                        #              
                ...
                if observable in exp['ignition_delay_observables']:
                    if re.match('[Ss]hock [Tt]ube',exp['simulation_type']):
                        if len(exp['simulation'].temperatures)>1:
                            ...
                            # plt.savefig(os.path.join(self.working_directory,'Experiment_'+str(i+1)+'_'+str(observable)+'_'+str(self.exp_dict_list_original[i]['simulation'].temperature)+'K_'+sp+'.pdf'), bbox_inches='tight',dpi=1000)
                            # plt.savefig(os.path.join(self.working_directory,'Experiment_'+str(i+1)+'_'+str(observable)+'_'+str(self.exp_dict_list_original[i]['simulation'].temperature)+'K_'+sp+'.svg'), bbox_inches='tight',dpi=1000,transparent=True)
                            # temp_df_for_species.to_csv(self.working_directory+'/'+'Experiment_'+str(i+1)+'_'+str(observable)+'_'+str(self.exp_dict_list_original[i]['simulation'].temperature)+'K_'+sp+'.csv',index=False) 
                    ...
                    if bool(sigmas_optimized)==True:
                        ...
                        if len(data_df) != len(unweighted_df):
                            ...
                            if self.shade_optimized_error == True:
                                ...
                                for ii in range(1,self.shade_steps+1):
                                    top = exp['absorbance_model_data'][wl] + (high_error_optimized-exp['absorbance_model_data'][wl])/self.shade_steps*ii
                                    bottom = exp['absorbance_model_data'][wl] - (exp['absorbance_model_data'][wl]-low_error_optimized)/self.shade_steps*ii
                                    plt.fill_between(exp['absorbance_experimental_data'][k]['time']*1e3, bottom, top, color='gray', alpha=1/self.shade_steps)                          
...
#                        plt.ylim(top=.35)
                    ...
                    # plt.savefig(self.working_directory+'/'+'Experiment_'+str(i+1)+' '+'Absorb at'+'_'+str(wl)+'_'+str(self.exp_dict_list_original[i]['simulation'].temperature)+'K_'+sp+'.pdf', bbox_inches='tight')
                    # plt.savefig(self.working_directory+'/'+'Experiment_'+str(i+1)+' '+'Absorb at'+'_'+str(wl)+'_'+str(self.exp_dict_list_original[i]['simulation'].temperature)+'K_'+sp+'.svg', bbox_inches='tight',transparent=True)


                    if self.pdf == True:
                        if self.dpi == None:
                            plt.savefig(self.working_directory+'/'+'Experiment_'+str(i+1)+'_Absorb_at'+'_'+str(wl)+'.pdf', bbox_inches='tight')
                        else:
                            plt.savefig(self.working_directory+'/'+'Experiment_'+str(i+1)+'_Absorb_at'+'_'+str(wl)+'.pdf', bbox_inches='tight',dpi=1000)
                    if self.png == True:
                        if self.dpi == None:                        
                            plt.savefig(self.working_directory+'/'+'Experiment_'+str(i+1)+'_Absorb_at'+'_'+str(wl)+'.png', bbox_inches='tight')
                        else:
                            plt.savefig(self.working_directory+'/'+'Experiment_'+str(i+1)+'_Absorb_at'+'_'+str(wl)+'.png', bbox_inches='tight',dpi=1000)
                    if self.svg == True:
                        if self.dpi == None:                                    
                            plt.savefig(self.working_directory+'/'+'Experiment_'+str(i+1)+'_Absorb_at'+'_'+str(wl)+'.svg', bbox_inches='tight',transparent=True)
                        else:
                            plt.savefig(self.working_directory+'/'+'Experiment_'+str(i+1)+'_Absorb_at'+'_'+str(wl)+'.svg', bbox_inches='tight',dpi=1000,transparent=True) 
                                


                    temp_optimized_df_for_species.to_csv(self.working_directory+'/'+'Experiment_'+str(i+1)+'_Absorb_optimized.csv',index=False) 
                    temp_optimized_error_df_for_species.to_csv(self.working_directory+'/'+'Experiment_'+str(i+1)+'_Absorb_error_optimized.csv',index=False) 
                    temp_original_df_for_species.to_csv(self.working_directory+'/'+'Experiment_'+str(i+1)+'_Absorb_original.csv',index=False)   
                    temp_original_error_df_for_species.to_csv(self.working_directory+'/'+'Experiment_'+str(i+1)+'_Absorb_error_original.csv',index=False)                     
...
#stubb
                ...
                plt.xlabel('Temperature [K]')
                plt.ylabel('k [kmol/m^3-s]')
                plt.title(reaction_list_from_mechanism[reaction])
                # plt.savefig(self.working_directory+'/'+'REACTION '+reaction_list_from_mechanism[reaction]+'.pdf', bbox_inches='tight')
                # plt.savefig(self.working_directory+'/'+'REACTION '+reaction_list_from_mechanism[reaction]+'.png', bbox_inches='tight')
                # plt.savefig(self.working_directory+'/'+'REACTION '+reaction_list_from_mechanism[reaction]+'.svg', bbox_inches='tight',transparent=True)
                

                if self.pdf == True:
                    if self.dpi == None:
                        plt.savefig(self.working_directory+'/'+'REACTION '+str(i+1)+'.pdf', bbox_inches='tight')
                    else:
                        plt.savefig(self.working_directory+'/'+'REACTION '+str(i+1)+'.pdf', bbox_inches='tight',dpi=1000)
                if self.png == True:
                    if self.dpi == None:                        
                        plt.savefig(self.working_directory+'/'+'REACTION '+str(i+1)+'.png', bbox_inches='tight')
                    else:
                        plt.savefig(self.working_directory+'/'+'REACTION '+str(i+1)+'.png', bbox_inches='tight',dpi=1000)
                if self.svg == True:
                    if self.dpi == None:                                    
                        plt.savefig(self.working_directory+'/'+'REACTION '+str(i+1)+'.svg', bbox_inches='tight',transparent=True)
                    else:
                        plt.savefig(self.working_directory+'/'+'REACTION '+str(i+1)+'.svg', bbox_inches='tight',dpi=1000,transparent=True)                 
                ...
                if 'perturbed_coef' in exp_dict_list_optimized[x].keys():
                    ...
                    if self.pdf == True:
                        if self.dpi == None:
                            ...
                            plt.savefig(self.working_directory+'/'+'Experiment_'+str(x+1)+' '+'Absorbance at'+'_'+str(wavelengths)+'.pdf', bbox_inches='tight',dpi=1000)
                    if self.png == True:
                        if self.dpi == None:                        
                            plt.savefig(self.working_directory+'/'+'Experiment_'+str(x+1)+' '+'Absorbance at'+'_'+str(wavelengths)+'.png', bbox_inches='tight')
                        else:
                            plt.savefig(self.working_directory+'/'+'Experiment_'+str(x+1)+' '+'Absorbance at'+'_'+str(wavelengths)+'.png', bbox_inches='tight',dpi=1000)
                    if self.svg == True:
                        if self.dpi == None:                                    
                            plt.savefig(self.working_directory+'/'+'Experiment_'+str(x+1)+' '+'Absorbance at'+'_'+str(wavelengths)+'.svg', bbox_inches='tight',transparent=True)
                        else:
                            plt.savefig(self.working_directory+'/'+'Experiment_'+str(x+1)+' '+'Absorbance at'+'_'+str(wavelengths)+'.svg', bbox_inches='tight',dpi=1000,transparent=True)                          
                    ...
                    if self.pdf == True:
                        if self.dpi == None:
                            plt.savefig(self.working_directory+'/'+'Experiment_'+str(x+1)+'_observable_'+exp_dict_list_optimized[0]['observables'][observable_counter]+'.pdf', bbox_inches='tight')
                        else:
                            plt.savefig(self.working_directory+'/'+'Experiment_'+str(x+1)+'_observable_'+exp_dict_list_optimized[0]['observables'][observable_counter]+'.pdf', bbox_inches='tight',dpi=1000)
                    if self.png == True:
                        if self.dpi == None:                        
                            plt.savefig(self.working_directory+'/'+'Experiment_'+str(x+1)+'_observable_'+exp_dict_list_optimized[0]['observables'][observable_counter]+'.png', bbox_inches='tight')
                        else:
                            plt.savefig(self.working_directory+'/'+'Experiment_'+str(x+1)+'_observable_'+exp_dict_list_optimized[0]['observables'][observable_counter]+'.png', bbox_inches='tight',dpi=1000)
                    if self.svg == True:
                        if self.dpi == None:                                    
                            plt.savefig(self.working_directory+'/'+'Experiment_'+str(x+1)+'_observable_'+exp_dict_list_optimized[0]['observables'][observable_counter]+'.svg', bbox_inches='tight',transparent=True)
                        else:
                            plt.savefig(self.working_directory+'/'+'Experiment_'+str(x+1)+'_observable_'+exp_dict_list_optimized[0]['observables'][observable_counter]+'.svg', bbox_inches='tight',dpi=1000,transparent=True)                       
    ...
    def sort_top_Sdx(self,top_sensitivity=10):
        ...
        length = sum(flat_list)
        observables_list = self.Ydf['value'].tolist()[length:]
        # observables_list = self.summed_observable_list_for_legend[length:]
        ...
        for x in range(len(self.simulation_lengths_of_experimental_data)):
            for y in range(len(self.simulation_lengths_of_experimental_data[x])):           
                ...
                A_temp_df = temp_df.filter(regex='A_')
                n_temp_df = temp_df.filter(regex='n_')
                Ea_temp_df = temp_df.filter(regex='Ea_')
                ...
                sensitivitys[x].append(ccn)           
                start = start + self.simulation_lengths_of_experimental_data[x][y]
        return sensitivitys,topSensitivities,new_temp_df.columns,len(A_temp_df.T)
    ...
    def get_observables_list(self):
        ...
        if bool(sigma_csv):
            ...
            Sig = np.array(Sig)
            Sig = Sig.reshape((Sig.shape[0],1))
            # observables_list  = df['Observable'].tolist()
            self.sigma_list = Sig
        ...
        observable_list_for_legend = []
        for obs in observables_list:
            lst = obs.split('_')
            if  len(lst) == 1:
                observable_list_for_legend.append(lst[0])
            else:
                if lst[0] =='A':
                    reaction_indx = int(lst[1])
                    reaction = reaction_equations[reaction_indx]
                    # observable_list_for_legend.append('A$_{'+reaction+'}$')
                    observable_list_for_legend.append('A '+reaction)
                elif lst[0] =='n':
                    reaction_indx = int(lst[1])
                    reaction = reaction_equations[reaction_indx]
                    # observable_list_for_legend.append('n$_{'+reaction+'}$') 
                    observable_list_for_legend.append('n '+reaction)                    
                elif lst[0] =='Ea':
                    reaction_indx = int(lst[1])
                    reaction = reaction_equations[reaction_indx]
                    # observable_list_for_legend.append('Ea$_{'+reaction+'}$') 
                    observable_list_for_legend.append('Ea '+reaction) 
                elif lst[1] == 'P':
                    reaction = lst[0]
                    parameter_number = int(lst[2])
                    # theory_label = self.theory_parameters_df[reaction][parameter_number].split('_')[1] + '_' + self.theory_parameters_df[reaction][parameter_number].split('_')[0]
                    theory_label = self.theory_parameters_df[reaction][parameter_number]
                    # observable_list_for_legend.append(self.theory_parameters_df[reaction][parameter_number]+'$_{'+reaction+'}$') 
                    observable_list_for_legend.append(theory_label+' '+reaction) 
                elif lst[0] == 'X':
                    experiment_number = int(lst[-1])
                    reactants = self.parsed_yaml_list_original[0][experiment_number]['speciesNames']
                    reactant_number = int(lst[1])
                    # observable_list_for_legend.append(lst[0]+'$_{'+reactants[reactant_number]+'}$')  
                    observable_list_for_legend.append(lst[0]+' '+reactants[reactant_number])                     
                elif lst[0] == 'Temperature':
                    if len(lst) == 3:
                        experiment_number = int(lst[-1])
                        temperature = self.parsed_yaml_list_original[0][experiment_number]['temperature']                 
                        # observable_list_for_legend.append(lst[0]+'$_{'+str(temperature)+' K}$') 
                        observable_list_for_legend.append(lst[0]+' '+str(temperature))  
                    elif len(lst) == 4:
                        experiment_number = int(lst[-1])
                        # print(experiment_number)
                        # print(self.parsed_yaml_list_original)
                        # print(self.parsed_yaml_list_original[experiment_number])
                        # print(self.parsed_yaml_list_original[experiment_number]['temperatures'])
                        temperatures = self.parsed_yaml_list_original[0][experiment_number]['temperatures']
                        temperature_number = int(lst[1])   
                        print(temperatures[temperature_number])                     
                        # observable_list_for_legend.append(lst[0]+'$_{'+str(temperatures[temperature_number])+' K}$')  
                        observable_list_for_legend.append(lst[0]+' '+str(temperatures[temperature_number]))                          
                elif lst[0] == 'Pressure':
                    if len(lst) == 3:
                        observable_list_for_legend.append(lst[0])  
                    elif len(lst) == 4:
                        observable_list_for_legend.append(lst[0]+' '+lst[1])       
                elif lst[0] == 'TimeShift':
                    if len(lst) == 3:
                        observable_list_for_legend.append(lst[0])  
                    elif len(lst) == 4:
                        observable_list_for_legend.append(lst[0]+' '+lst[1])       
                elif lst[0] == 'ResidenceTime':
                    if len(lst) == 3:
                        observable_list_for_legend.append(lst[0])  
                    elif len(lst) == 4:
                        observable_list_for_legend.append(lst[0]+' '+lst[1])   
        ...
        summed_observable_list_for_legend = []

        for obs in observables_list:
            lst = obs.split('_')
            if  len(lst) == 1:
                summed_observable_list_for_legend.append(lst[0])
            else:
                if lst[0] =='A':
                    reaction_indx = int(lst[1])
                    reaction = reaction_equations[reaction_indx]
                    summed_observable_list_for_legend.append('k '+reaction)
                elif lst[0] =='n':
                    pass               
                elif lst[0] =='Ea':
                    pass
                    ...
                    # theory_label = self.theory_parameters_df[reaction][parameter_number].split('_')[1] + '_' + self.theory_parameters_df[reaction][parameter_number].split('_')[0]
                    theory_label = self.theory_parameters_df[reaction][parameter_number]
                    # summed_observable_list_for_legend.append(self.theory_parameters_df[reaction][parameter_number]+'$_{'+reaction+'}$') 
                    summed_observable_list_for_legend.append(theory_label+' '+reaction) 
                elif lst[0] == 'X':
                    experiment_number = int(lst[-1])
                    reactants = self.parsed_yaml_list_original[0][experiment_number]['speciesNames']
                    reactant_number = int(lst[1])
                    summed_observable_list_for_legend.append(lst[0]+' '+reactants[reactant_number])                     
                elif lst[0] == 'Temperature':
                    if len(lst) == 3:
                        experiment_number = int(lst[-1])
                        temperature = self.parsed_yaml_list_original[0][experiment_number]['temperature']                 
                        summed_observable_list_for_legend.append(lst[0]+' '+str(temperature))  
                    elif len(lst) == 4:
                        experiment_number = int(lst[-1])
                        temperatures = self.parsed_yaml_list_original[0][experiment_number]['temperatures']
                        temperature_number = int(lst[1])   
                        print(temperatures[temperature_number])                     
                        summed_observable_list_for_legend.append(lst[0]+' '+str(temperatures[temperature_number]))                          
                elif lst[0] == 'Pressure':
                    if len(lst) == 3:
                        summed_observable_list_for_legend.append(lst[0])  
                    elif len(lst) == 4:
                        summed_observable_list_for_legend.append(lst[0]+' '+lst[1])       
                elif lst[0] == 'TimeShift':
                    if len(lst) == 3:
                        summed_observable_list_for_legend.append(lst[0])  
                    elif len(lst) == 4:
                    ...
                    if len(lst) == 3:
                        ...
                        summed_observable_list_for_legend.append(lst[0]+' '+lst[1])     
        self.summed_observable_list_for_legend = summed_observable_list_for_legend          
                        
                        
            # py_list = []
            # for i, py in enumerate(self.parsed_yaml_list_original):
        ...
        for obs in observables_list:
            lst = obs.split('_')
            if  len(lst) == 1:
                observable_list_transformed.append(lst[0])
            else:                
                if lst[0] =='A':
                    reaction_indx = int(lst[1])
                    reaction = reaction_equations[reaction_indx]
                    observable_list_transformed.append('A '+reaction)
                elif lst[0] =='n':
                    reaction_indx = int(lst[1])
                    reaction = reaction_equations[reaction_indx]
                    observable_list_transformed.append('n '+reaction)                    
                elif lst[0] =='Ea':
                    reaction_indx = int(lst[1])
                    reaction = reaction_equations[reaction_indx]
                    observable_list_transformed.append('Ea '+reaction) 
                elif lst[1] == 'P':
                    reaction = lst[0]
                    parameter_number = int(lst[2])
                    observable_list_transformed.append(self.theory_parameters_df[reaction][parameter_number]+' '+reaction) 
                elif lst[0] == 'X':
                    experiment_number = int(lst[-1])
                    reactants = self.parsed_yaml_list_original[0][experiment_number]['speciesNames']
                    yaml_name = os.path.basename(self.files_to_include[0][experiment_number][0])[:-5]
                    reactant_number = int(lst[1])
                    observable_list_transformed.append(lst[0]+'_'+reactants[reactant_number] + ' ' + yaml_name)                     
                # elif lst[0] == 'Temperature':
                #     if len(lst) == 3:
                #         experiment_number = int(lst[-1])
                #         temperature = self.parsed_yaml_list_original[0][experiment_number]['temperature']                 
                #         observable_list_for_Xdf.append(lst[0]+' '+str(temperature)+' K')  
                #     elif len(lst) == 4:
                #         experiment_number = int(lst[-1])
                #         # print(experiment_number)
                #         # print(self.parsed_yaml_list_original)
                ...
                #         temperature_number = int(lst[1])   
                #         print(temperatures[temperature_number])                     
                #         observable_list_for_Xdf.append(lst[0]+' '+str(temperatures[temperature_number])+' K')                          
                # elif lst[0] == 'Pressure':
                #     if len(lst) == 3:
                #         observable_list_for_Xdf.append(lst[0])  
                #     elif len(lst) == 4:
                #         observable_list_for_Xdf.append(lst[0]+' '+lst[1])       
                # elif lst[0] == 'TimeShift':
                #     if len(lst) == 3:
                #         observable_list_for_Xdf.append(lst[0])  
                #     elif len(lst) == 4:
                ...
                #         observable_list_for_Xdf.append(lst[0]+' '+lst[1])  
                    ...
                    experiment_number = int(lst[-1])
                    yaml_name = os.path.basename(self.files_to_include[0][experiment_number][0])[:-5]
                    observable_list_transformed.append(lst[0] + ' ' + yaml_name)                      
                else:
                    experiment_number = int(lst[-1])
                    yaml_name = os.path.basename(self.files_to_include[0][experiment_number][0])[:-5]
                    if len(lst) == 3:
                        ...
                        observable_list_transformed.append(lst[0]+'_'+ lst[1] + ' ' + yaml_name)     
                    
                    # observable_list_for_Xdf.append(obs)
    ...
    def plotting_uncertainty_weighted_sens(self):
        ...
        observables_list_for_legend = self.observable_list_for_legend
        ...
        def subplot_function(number_of_observables_in_simulation,time_profiles,sensitivities,top_sensitivity_single_exp,observables_list_for_legend,list_of_experiment_observables,experiment_number):
            for plot_number in range(number_of_observables_in_simulation):
                ...
                UWSA_df.to_csv(self.working_directory+'/'+'Experiment'+ '_' +str(experiment_number+1)+'_UWSA'+'_'+str(list_of_experiment_observables[plot_number])+'.csv',index=False)   
                if self.simulation_run==None:
                    if self.pdf == True:
                        if self.dpi == None:
                            plt.savefig(self.working_directory+'/'+'Experiment'+ '_' +str(experiment_number+1)+'_UWSA'+'_'+str(list_of_experiment_observables[plot_number])+'.pdf', bbox_inches='tight')
                        else:
                            plt.savefig(self.working_directory+'/'+'Experiment'+ '_' +str(experiment_number+1)+'_UWSA'+'_'+str(list_of_experiment_observables[plot_number])+'.pdf', bbox_inches='tight',dpi=1000)
                    if self.png == True:
                        if self.dpi == None:                        
                            plt.savefig(self.working_directory+'/'+'Experiment'+ '_' +str(experiment_number+1)+'_UWSA'+'_'+str(list_of_experiment_observables[plot_number])+'.png', bbox_inches='tight')
                        else:
                            plt.savefig(self.working_directory+'/'+'Experiment'+ '_' +str(experiment_number+1)+'_UWSA'+'_'+str(list_of_experiment_observables[plot_number])+'.png', bbox_inches='tight',dpi=1000)
                    if self.svg == True:
                        if self.dpi == None:                                    
                            plt.savefig(self.working_directory+'/'+'Experiment'+ '_' +str(experiment_number+1)+'_UWSA'+'_'+str(list_of_experiment_observables[plot_number])+'.svg', bbox_inches='tight',transparent=True)
                        else:
                            plt.savefig(self.working_directory+'/'+'Experiment'+ '_' +str(experiment_number+1)+'_UWSA'+'_'+str(list_of_experiment_observables[plot_number])+'.svg', bbox_inches='tight',dpi=1000,transparent=True)   
                else:
                    plt.title('Experiment_'+str(self.simulation_run) + ' (' + os.path.basename(self.files_to_include[0][experiment_number][0])[:-5] + ')')
                    if self.pdf == True:
                        if self.dpi == None:
                            plt.savefig(self.working_directory+'/'+'Experiment'+ '_' +str(self.simulation_run)+'_UWSA'+'_'+str(list_of_experiment_observables[plot_number])+'.pdf', bbox_inches='tight')
                        else:
                            plt.savefig(self.working_directory+'/'+'Experiment'+ '_' +str(self.simulation_run)+'_UWSA'+'_'+str(list_of_experiment_observables[plot_number])+'.pdf', bbox_inches='tight',dpi=1000)
                    if self.png == True:
                        if self.dpi == None:                        
                            plt.savefig(self.working_directory+'/'+'Experiment'+ '_' +str(self.simulation_run)+'_UWSA'+'_'+str(list_of_experiment_observables[plot_number])+'.png', bbox_inches='tight')
                        else:
                            plt.savefig(self.working_directory+'/'+'Experiment'+ '_' +str(self.simulation_run)+'_UWSA'+'_'+str(list_of_experiment_observables[plot_number])+'.png', bbox_inches='tight',dpi=1000)
                    if self.svg == True:
                        if self.dpi == None:                                    
                            plt.savefig(self.working_directory+'/'+'Experiment'+ '_' +str(self.simulation_run)+'_UWSA'+'_'+str(list_of_experiment_observables[plot_number])+'.svg', bbox_inches='tight',transparent=True)
                        else:
                            plt.savefig(self.working_directory+'/'+'Experiment'+ '_' +str(self.simulation_run)+'_UWSA'+'_'+str(list_of_experiment_observables[plot_number])+'.svg', bbox_inches='tight',dpi=1000,transparent=True)           
    ...
    def plotting_Sdx(self):
        ...
        # observables_list = self.get_observables_list()
        observables_list_for_legend = self.observable_list_for_legend
        summed_observables_list_for_legend = self.summed_observable_list_for_legend
        if bool(self.sigma_uncertainty_weighted_sensitivity_csv):
            sigma_list = self.sigma_list
        else:
            sigma_list = list(self.short_sigma)
        ...
        def subplot_function(number_of_observables_in_simulation,time_profiles,sensitivities,top_sensitivity_single_exp,observables_list_for_legend,list_of_experiment_observables,experiment_number):
            for plot_number in range(number_of_observables_in_simulation):
                ...
                for c,top_columns in enumerate(top_sensitivity_single_exp[plot_number]):
                    ...
                    top_columns_index = list(summed_observable_list).index(top_columns)
                    ...
                    if c < 10:
                        if top_columns.split('_')[0] == 'k':
                            ...
                            label = summed_observables_list_for_legend[top_columns_index] +' [A:'+str(sigma_A)+', n:'+str(sigma_n)+', Ea:'+str(sigma_Ea)+', '+str(summed_X)+']') 
                        else:
                            plt.plot(time_profiles[plot_number],sensitivities[plot_number][:,c],
                            label = summed_observables_list_for_legend[top_columns_index] +' ['+str(sigma_list[top_columns_index+2*num_rxn][0])+', '+str(X[top_columns_index+2*num_rxn][0])+']') 
                    observable_ylabel = list_of_experiment_observables[plot_number]
                    observable_ylabel_string = list(observable_ylabel)
                    for i, oys in enumerate(observable_ylabel_string):
                        if oys.isdigit() == True:
                    ...
                    if top_columns.split('_')[0] == 'k':
                        ...
                        sigma_n = sigma_list[top_columns_index+num_rxn][0]
                        sigma_Ea = sigma_list[top_columns_index+2*num_rxn][0]
                        Sdx_df[summed_observables_list_for_legend[top_columns_index] +' [A:'+str(sigma_A)+', n:'+str(sigma_n)+', Ea:'+str(sigma_Ea)+', '+str(summed_X)+']'] = pd.Series(sensitivities[plot_number][:,c])  
                    else:
                        Sdx_df[summed_observables_list_for_legend[top_columns_index] +' ['+str(sigma_list[top_columns_index+2*num_rxn][0])+', '+str(X[top_columns_index+2*num_rxn][0])+']'] = pd.Series(sensitivities[plot_number][:,c])                     
                    # Sdx_df[summed_observables_list_for_legend[top_columns_index] +' '+str(sigma_list[top_columns])] = pd.Series(sensitivities[plot_number][:,c])   
                Sdx_df.to_csv(self.working_directory+'/'+'Experiment'+ '_' +str(experiment_number+1)+'_Sdx'+'_'+str(list_of_experiment_observables[plot_number])+'.csv',index=False)   
                if self.simulation_run==None:
                    if self.pdf == True:
                        if self.dpi == None:
                            plt.savefig(self.working_directory+'/'+'Experiment'+ '_' +str(experiment_number+1)+'_Sdx'+'_'+str(list_of_experiment_observables[plot_number])+'.pdf', bbox_inches='tight')
                        else:
                            plt.savefig(self.working_directory+'/'+'Experiment'+ '_' +str(experiment_number+1)+'_Sdx'+'_'+str(list_of_experiment_observables[plot_number])+'.pdf', bbox_inches='tight',dpi=1000)
                    if self.png == True:
                        if self.dpi == None:                        
                            plt.savefig(self.working_directory+'/'+'Experiment'+ '_' +str(experiment_number+1)+'_Sdx'+'_'+str(list_of_experiment_observables[plot_number])+'.png', bbox_inches='tight')
                        else:
                            plt.savefig(self.working_directory+'/'+'Experiment'+ '_' +str(experiment_number+1)+'_Sdx'+'_'+str(list_of_experiment_observables[plot_number])+'.png', bbox_inches='tight',dpi=1000)
                    if self.svg == True:
                        if self.dpi == None:                                    
                            plt.savefig(self.working_directory+'/'+'Experiment'+ '_' +str(experiment_number+1)+'_Sdx'+'_'+str(list_of_experiment_observables[plot_number])+'.svg', bbox_inches='tight',transparent=True)
                        else:
                            plt.savefig(self.working_directory+'/'+'Experiment'+ '_' +str(experiment_number+1)+'_Sdx'+'_'+str(list_of_experiment_observables[plot_number])+'.svg', bbox_inches='tight',dpi=1000,transparent=True)   
                else:
                    plt.title('Experiment_'+str(self.simulation_run) + ' (' + os.path.basename(self.files_to_include[0][experiment_number][0])[:-5] + ')')
                    if self.pdf == True:
                        if self.dpi == None:
                            plt.savefig(self.working_directory+'/'+'Experiment'+ '_' +str(self.simulation_run)+'_Sdx'+'_'+str(list_of_experiment_observables[plot_number])+'.pdf', bbox_inches='tight')
                        else:
                            plt.savefig(self.working_directory+'/'+'Experiment'+ '_' +str(self.simulation_run)+'_Sdx'+'_'+str(list_of_experiment_observables[plot_number])+'.pdf', bbox_inches='tight',dpi=1000)
                    if self.png == True:
                        if self.dpi == None:                        
                            plt.savefig(self.working_directory+'/'+'Experiment'+ '_' +str(self.simulation_run)+'_Sdx'+'_'+str(list_of_experiment_observables[plot_number])+'.png', bbox_inches='tight')
                        else:
                            plt.savefig(self.working_directory+'/'+'Experiment'+ '_' +str(self.simulation_run)+'_Sdx'+'_'+str(list_of_experiment_observables[plot_number])+'.png', bbox_inches='tight',dpi=1000)
                    if self.svg == True:
                        if self.dpi == None:                                    
                            plt.savefig(self.working_directory+'/'+'Experiment'+ '_' +str(self.simulation_run)+'_Sdx'+'_'+str(list_of_experiment_observables[plot_number])+'.svg', bbox_inches='tight',transparent=True)
                        else:
                            plt.savefig(self.working_directory+'/'+'Experiment'+ '_' +str(self.simulation_run)+'_Sdx'+'_'+str(list_of_experiment_observables[plot_number])+'.svg', bbox_inches='tight',dpi=1000,transparent=True)           
        for x in range(len(sensitivities)):            
            number_of_observables_in_simulation = len(sensitivities[x])
            subplot_function(number_of_observables_in_simulation,time_profiles[x],sensitivities[x],top_sensitivities[x],observables_list_for_legend,list_of_experiment_observables[x],x)
        return 
...
################################################################################        
               ...
               # print(reaction_list_from_mechanism_original.index(reaction_list_from_mechanism[reaction]))
                ...
                if self.pdf == True:
                    if self.dpi == None:
                        ...
                        plt.savefig(os.path.join(self.working_directory,reaction_list_from_mechanism[reaction]+'.pdf'), bbox_inches='tight',dpi=1000)
                if self.png == True:
                    if self.dpi == None:                        
                        plt.savefig(os.path.join(self.working_directory,reaction_list_from_mechanism[reaction]+'.png'), bbox_inches='tight')
                    else:
                        plt.savefig(os.path.join(self.working_directory,reaction_list_from_mechanism[reaction]+'.png'), bbox_inches='tight',dpi=1000)
                if self.svg == True:
                    if self.dpi == None:                                    
                        plt.savefig(os.path.join(self.working_directory,reaction_list_from_mechanism[reaction]+'.svg'), bbox_inches='tight',transparent=True)
                    else:
                        plt.savefig(os.path.join(self.working_directory,reaction_list_from_mechanism[reaction]+'.svg'), bbox_inches='tight',dpi=1000,transparent=True)      
    ...
    def plotting_normal_distributions(self,
        ...
        for parameter in paramter_list:
            ...
            plt.savefig(self.working_directory+'/'+parameter+'_distribution'+'.pdf',bbox_inches='tight',dpi=1000)
            plt.savefig(self.working_directory+'/'+parameter+'_distribution'+'.png',bbox_inches='tight',dpi=1000)
            plt.savefig(self.working_directory+'/'+parameter+'_distribution'+'.svg',bbox_inches='tight',dpi=1000,transparent=True)
...
#            CS = plt.contour(Z_test, [z1, z2, z3], origin='lower', extent=(-50,50,-50,50),colors='red')            
            ...
            if bool(joint_data_csv):
                ...
                plt.savefig(self.working_directory+'/'+couple[0]+'_'+couple[1]+'_distribution'+'.pdf',bbox_inches='tight',dpi=1000)
                plt.savefig(self.working_directory+'/'+couple[0]+'_'+couple[1]+'_distribution'+'.png',bbox_inches='tight',dpi=1000)
                plt.savefig(self.working_directory+'/'+couple[0]+'_'+couple[1]+'_distribution'+'.svg',bbox_inches='tight',dpi=1000,transparent=True)
...
###################################################################################################################################################################################################################  
    ...
    #                        plt.ylim(top=.35)
                        ...
                        plt.savefig(self.working_directory+'/'+'Experiment_'+str(experiment_number_want_to_plot)+'_three_figure_obervables.pdf', bbox_inches='tight',dpi=1000)
                        plt.savefig(self.working_directory+'/'+'Experiment_'+str(experiment_number_want_to_plot)+'_three_figure_obervables.png', bbox_inches='tight',dpi=1000)
                        plt.savefig(self.working_directory+'/'+'Experiment_'+str(experiment_number_want_to_plot)+'_three_figure_obervables.svg', bbox_inches='tight',dpi=1000,transparent=True)
                        plt.tick_params(direction='in')
        ...
        def sort_rate_constant_target_values(parsed_csv,unique_reactions,gas):
            ...
            for i,reaction in enumerate(parsed_csv['Reaction']):
                if '/' in reaction:
                    ...
                    idx = list(reaction_number_in_cti_file_with_these_reactants)
                    target_value_ks[unique_reactions.index(idx)].append(parsed_csv['k'][i])  
                    target_value_temps[unique_reactions.index(idx)].append(parsed_csv['temperature'][i])   
                    if 'ref' in list(parsed_csv.columns):                 
        ...
        def calculate_sigmas_for_rate_constants(k_target_value_S_matrix,k_target_values_parsed_csv,unique_reactions,gas,covarience):
            ...
            for row in range(shape[0]):
                ...
                if '/' in k_target_values_parsed_csv['Reaction'][row]:
                    ...
                    indx = list(reaction_number_in_cti_file_with_these_reactants)
                    sigma_list_for_target_ks[unique_reactions.index(indx)].append(sigma_k)  
        ...
        def calculating_target_value_ks_from_cantera_for_sigmas(k_target_values_parsed_csv,gas,unique_reactions):
            ...
            for i,reaction in enumerate(target_reactions): 
                if '/' in reaction:
                    ...
                    # k_s=[]    
                    # for reac in list_of_reactions_cleaned:
                    reaction_number_in_cti = reactions_in_cti_file.index(list_of_reactions_cleaned[0])
                    reactions_in_cti_file_with_these_reactants.append(list_of_reactions_cleaned[0])
                    reaction_number_in_cti_file_with_these_reactants.append(reaction_number_in_cti)
                    coeff_sum = sum(gas.reaction(reaction_number_in_cti).reactants.values())
                    k_num = gas.forward_rate_constants[reaction_number_in_cti]
                    
                    reaction_number_in_cti = reactions_in_cti_file.index(list_of_reactions_cleaned[1])
                    reactions_in_cti_file_with_these_reactants.append(list_of_reactions_cleaned[1])
                    reaction_number_in_cti_file_with_these_reactants.append(reaction_number_in_cti)
                    coeff_sum = sum(gas.reaction(reaction_number_in_cti).reactants.values())
                    k_den = gas.forward_rate_constants[reaction_number_in_cti]                    
        ...
        #edit here to skip plotting ?
            ...
            for row in range(k_target_value_csv_extra.shape[0]):
                if '/' in k_target_value_csv_extra['Reaction'][row]:
                    ...
                    unique_reactions_optimized.append(list(reaction_number_in_cti_file_with_these_reactants))
                    unique_reactions_original.append(list(reaction_number_in_cti_file_with_these_reactants_original))  
           ...
           #  ######################  
            ...
            target_value_temps_optimized,target_value_ks_optimized,target_value_refs_optimized = sort_rate_constant_target_values(k_target_value_csv_extra,unique_reactions_optimized,gas_optimized)
            target_value_temps_original,target_value_ks_original,target_value_refs_original = sort_rate_constant_target_values(k_target_value_csv_extra,unique_reactions_original,gas_original)
            ...
            for row in range(k_target_value_csv.shape[0]):
                if '/' in k_target_value_csv['Reaction'][row]:
                    ...
                    for reaction in list_of_reactions_cleaned:
                        ...
                        reaction_number_in_cti_file_with_these_reactants_original.append(reaction_number_in_cti_file)  
                    unique_reactions_optimized_for_plotting.append(list(reaction_number_in_cti_file_with_these_reactants))
                    unique_reactions_original_for_plotting.append(list(reaction_number_in_cti_file_with_these_reactants_original))       
           ...
           # #############################################
            ...
            for i,reaction in enumerate(unique_reactions_optimized):
                ...
                optimized_rate_constant_df.to_csv(self.working_directory+'/'+'Rate_Constant_'+str(i+1)+'_optimized.csv',index=False) 
                ...
                if type(reaction)==list:
                    ...
                    # new_tuple = list(sorted(new_tuple))
                ...
                original_rate_constant_df.to_csv(self.working_directory+'/'+'Rate_Constant_'+str(i+1)+'_original.csv',index=False) 
                ...
                if type(reaction) == list:
                    ...
                    # reactants = ttl_reaction_string[0].split('<=>')[0].rstrip().lstrip().replace(' (+M)','')
                    # r_num = reaction.split('/') 
                    plt.title(reaction_list_from_mechanism[reaction[0]] + ' / ' + reaction_list_from_mechanism[reaction[1]])
                    plt.legend(ncol=2)
                    if temperature_range_to_plot_over is not None:
                        plt.xlim(temperature_range_to_plot_over[0],temperature_range_to_plot_over[1])
                        plt.ylim(low_value_axis,high_value_axis)
                    # plt.savefig(self.working_directory+'/'+'REACTION_COMBINED_'+reactants+'.pdf', bbox_inches='tight')
                    # plt.savefig(self.working_directory+'/'+'REACTION_COMBINED_'+reactants+'.png', bbox_inches='tight')
                    # plt.savefig(self.working_directory+'/'+'REACTION_COMBINED_'+reactants+'.svg', bbox_inches='tight',transparent=True)  
                    
                    if self.pdf == True:
                        if self.dpi == None:
                            plt.savefig(self.working_directory+'/'+'Rate_Constant_'+str(i+1)+'.pdf', bbox_inches='tight')
                        else:
                            plt.savefig(self.working_directory+'/'+'Rate_Constant_'+str(i+1)+'.pdf', bbox_inches='tight',dpi=1000)
                    if self.png == True:
                        if self.dpi == None:                        
                            plt.savefig(self.working_directory+'/'+'Rate_Constant_'+str(i+1)+'.png', bbox_inches='tight')
                        else:
                            plt.savefig(self.working_directory+'/'+'Rate_Constant_'+str(i+1)+'.png', bbox_inches='tight',dpi=1000)
                    if self.svg == True:
                        if self.dpi == None:                                    
                            plt.savefig(self.working_directory+'/'+'Rate_Constant_'+str(i+1)+'.svg', bbox_inches='tight',transparent=True)
                        else:
                            plt.savefig(self.working_directory+'/'+'Rate_Constant_'+str(i+1)+'.svg', bbox_inches='tight',dpi=1000,transparent=True)          
                    ...
                    if reaction==(309, 310):
                        ...
                        if temperature_range_to_plot_over is not None:
                            plt.xlim(temperature_range_to_plot_over[0],temperature_range_to_plot_over[1])
                            plt.ylim(low_value_axis,high_value_axis)
                        # plt.savefig(self.working_directory+'/'+'REACTION_COMBINED_'+ttl_reaction_string[0]+'_+_'+ttl_reaction_string[1]+'.pdf', bbox_inches='tight')
                        # plt.savefig(self.working_directory+'/'+'REACTION_COMBINED_'+ttl_reaction_string[0]+'_+_'+ttl_reaction_string[1]+'.png', bbox_inches='tight')                        
                        # plt.savefig(self.working_directory+'/'+'REACTION_COMBINED_'+ttl_reaction_string[0]+'_+_'+ttl_reaction_string[1]+'.svg', bbox_inches='tight',transparent=True)           
                    ...
                    # plt.savefig(self.working_directory+'/'+'REACTION_COMBINED_'+reaction_list_from_mechanism[reaction]+'.pdf', bbox_inches='tight')
                    # plt.savefig(self.working_directory+'/'+'REACTION_COMBINED_'+reaction_list_from_mechanism[reaction]+'.png', bbox_inches='tight')
                    # plt.savefig(self.working_directory+'/'+'REACTION_COMBINED_'+reaction_list_from_mechanism[reaction]+'.svg', bbox_inches='tight',transparent=True)
    ...
    def plotting_uncertainty_weighted_sens_rate_constant(self,top_sensitivity=10,
        ...
        for reaction_number in self.unique_reactions_original:
            ...
            if type(reaction_number) == list:
                ...
                print(temp_tuple)
                list_of_reaction_strings.append(temp_tuple[0] + ' / ' + temp_tuple[1])            
        ...
        for i,reaction in enumerate(self.unique_reactions_original):
            ...
            UWSA_Rate_Constant_df.to_csv(self.working_directory+'/'+'Rate_Constant_'+str(i+1)+'_UWSA.csv',index=False)   
            ...
            # plt.savefig(self.working_directory+'/'+'UWSA_'+str(list_of_reaction_strings[i])+'.pdf', bbox_inches='tight')
            # plt.savefig(self.working_directory+'/'+'UWSA_'+str(list_of_reaction_strings[i])+'.png', bbox_inches='tight')
            # plt.savefig(self.working_directory+'/'+'UWSA_'+str(list_of_reaction_strings[i])+'.svg', bbox_inches='tight',transparent=True)
            
            if self.pdf == True:
                if self.dpi == None:
                    plt.savefig(self.working_directory+'/'+'Rate_Constant_'+str(i+1)+'_UWSA'+'.pdf', bbox_inches='tight')
                else:
                    plt.savefig(self.working_directory+'/'+'Rate_Constant_'+str(i+1)+'_UWSA'+'.pdf', bbox_inches='tight',dpi=1000)
            if self.png == True:
                if self.dpi == None:                        
                    plt.savefig(self.working_directory+'/'+'Rate_Constant_'+str(i+1)+'_UWSA'+'.png', bbox_inches='tight')
                else:
                    plt.savefig(self.working_directory+'/'+'Rate_Constant_'+str(i+1)+'_UWSA'+'.png', bbox_inches='tight',dpi=1000)
            if self.svg == True:
                if self.dpi == None:                                    
                    plt.savefig(self.working_directory+'/'+'Rate_Constant_'+str(i+1)+'_UWSA'+'.svg', bbox_inches='tight',transparent=True)
                else:
                    plt.savefig(self.working_directory+'/'+'Rate_Constant_'+str(i+1)+'_UWSA'+'.svg', bbox_inches='tight',dpi=1000,transparent=True) 
    ...
    def plotting_Sdx_rate_constant(self,top_sensitivity=10,
        ...
        for x in range(len(length_of_temperature_vectors)):
            for y in range(len(length_of_temperature_vectors[x])):
                ...
                summed_observable_list.append(new_temp_df.columns)
        ...
        for i,reaction in enumerate(self.unique_reactions_original):
            ...
            for ccc, top_columns in enumerate(topSensitivities[i][0]):
                ...
                top_columns_index = list(summed_observable_list[i]).index(top_columns)                
                ...
                if ccc < top_sensitivity:
                    ...
                    if top_columns.split('_')[0] == 'k':
                        ...
                        plt.plot(c,d,
                        label = summed_observables_list_for_legend[top_columns_index] +' ['+str(Sig[top_columns_index+2*num_rxn][0])+', '+str(X[top_columns_index+2*num_rxn][0])+']') 
                ...
                if top_columns.split('_')[0] == 'k':
                    ...
                    Sdx_Rate_Constant_df[summed_observables_list_for_legend[top_columns_index] +' [A:'+str(sigma_A)+', n:'+str(sigma_n)+', Ea:'+str(sigma_Ea)+', '+str(summed_X)+']'] = pd.Series(sensitivitys[i][0][:,ccc])   
                else:           
                    Sdx_Rate_Constant_df[summed_observables_list_for_legend[top_columns_index] +' ['+str(Sig[top_columns_index+2*num_rxn][0])+', '+str(X[top_columns_index+2*num_rxn][0])+']'] = pd.Series(sensitivitys[i][0][:,ccc])   
                
            Sdx_Rate_Constant_df.to_csv(self.working_directory+'/'+'Rate_Constant_'+str(i+1)+'_Sdx.csv',index=False)   
            ...
            # plt.savefig(self.working_directory+'/'+'Sdx_'+str(list_of_reaction_strings[i])+'.pdf', bbox_inches='tight')
            # plt.savefig(self.working_directory+'/'+'Sdx_'+str(list_of_reaction_strings[i])+'.png', bbox_inches='tight')
            # plt.savefig(self.working_directory+'/'+'Sdx_'+str(list_of_reaction_strings[i])+'.svg', bbox_inches='tight',transparent=True)
            
            if self.pdf == True:
                if self.dpi == None:
                    plt.savefig(self.working_directory+'/'+'Rate_Constant_'+str(i+1)+'_Sdx'+'.pdf', bbox_inches='tight')
                else:
                    plt.savefig(self.working_directory+'/'+'Rate_Constant_'+str(i+1)+'_Sdx'+'.pdf', bbox_inches='tight',dpi=1000)
            if self.png == True:
                if self.dpi == None:                        
                    plt.savefig(self.working_directory+'/'+'Rate_Constant_'+str(i+1)+'_Sdx'+'.png', bbox_inches='tight')
                else:
                    plt.savefig(self.working_directory+'/'+'Rate_Constant_'+str(i+1)+'_Sdx'+'.png', bbox_inches='tight',dpi=1000)
            if self.svg == True:
                if self.dpi == None:                                    
                    plt.savefig(self.working_directory+'/'+'Rate_Constant_'+str(i+1)+'_Sdx'+'.svg', bbox_inches='tight',transparent=True)
                else:
                    plt.savefig(self.working_directory+'/'+'Rate_Constant_'+str(i+1)+'_Sdx'+'.svg', bbox_inches='tight',dpi=1000,transparent=True) 
